...
public:
	...
	// --- BLOCK BEGIN ---
	void BlockOnUnban(int ExecutorId, const char *pWhat) override;
	// --- BLOCK END ---
...
public:
	...
	// --- BLOCK BEGIN: engine-side Block helpers ---
	...
	const char *BlockExecutorName(int ExecutorId) const;
	// --- BLOCK END ---
	...
	// -- comes from here instead when m_BlockHasPubMap is set.
	bool m_BlockHasPubMap = false;
	...
	void BlockLoadPubMap(const char *pMapName);
	bool BlockHasPubMap() const { return m_BlockHasPubMap; }
	const SHA256_DIGEST &BlockMapSha256(int MapType) const { return m_BlockHasPubMap ? m_aBwPubMapSha256[MapType] : m_aCurrentMapSha256[MapType]; }
	unsigned BlockMapCrc(int MapType) const { return m_BlockHasPubMap ? m_aBwPubMapCrc[MapType] : m_aCurrentMapCrc[MapType]; }
	unsigned int BlockMapSize(int MapType) const { return m_BlockHasPubMap ? m_aBwPubMapSize[MapType] : m_aCurrentMapSize[MapType]; }
	const unsigned char *BlockMapData(int MapType) const { return m_BlockHasPubMap ? m_apBwPubMapData[MapType] : m_apCurrentMapData[MapType]; }
	...
	void BlockPreShutdownFlush(const char *pWhen);
	// True when a Block component refuses this client (VPN, port proxy).
	bool BlockComponentsRejectJoin(int ClientId);
	// Server-browser overrides: the Block clan and the account level.
	// Return the plain values when the game context is not a CGameContext.
	const char *BlockServerInfoClan(int ClientId);
	int BlockServerInfoScore(int ClientId, int Fallback);
	// --- BLOCK END ---
