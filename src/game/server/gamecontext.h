...
public:
	...
	void CreateExplosionVisual(vec2 Pos, CClientMask Mask = CClientMask().set()); // visual-only explosion: sound + particle, no force/damage
	...
	void SendChatClan(int ClanId, const char *pText, int VersionFlags = FLAG_SIX | FLAG_SIXUP, int From = -1) const;
