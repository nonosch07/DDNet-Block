...
void CPassiveZone::Tick()
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(InZone && !WasInZone && pChar->Core()->m_Passive)
		{
			...
			if(pChar->Bw().m_HookedBy >= 0 && pChar->Bw().m_HookedBy < MAX_CLIENTS)
			{
				if(CCharacter *pHooker = GameServer()->GetPlayerChar(pChar->Bw().m_HookedBy))
				{
...
void CPassiveZone::OnCharacterDeath(CCharacter *pCharacter)
{
	...
	pCharacter->Bw().Core().m_Passive = false;
	if(pCharacter->GetPlayer())
	{
		pCharacter->GetPlayer()->Bw().PromotePassiveIfPending();
	}
}
...
void CPassiveZone::HandleProtection(int ClientID, CPlayer *pPlayer, CCharacter *pChar, bool InZone, bool WasInZone)
{
	...
	bool HasLocal = (pPlayer->Bw().m_LocalPassiveDuration > 0);
	bool HasAccount = (pPlayer->Bw().IsLoggedIn() && pPlayer->Bw().GetPlayerPassive() > 0);
	bool Eligible = (HasLocal || HasAccount) && pPlayer->Bw().IsUsingPassiveProtection();
