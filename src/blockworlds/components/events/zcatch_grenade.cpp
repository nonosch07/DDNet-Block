	...
	CEventComponent(pGameServer)
{
	...
	m_SpawnPositions = GameServer()->Bw().ZoneManager()->GetNamedQuadCenters("zcg_spawn");
...
void CZCatchGrenadeEvent::CloseRegistration()
{
	...
	GameServer()->Bw().SendBroadcast(-1, " ", false);
...
void CZCatchGrenadeEvent::FinishEvent()
{
	...
	if(m_FinishReason == NATURAL)
	{
		if(m_Winner != -1)
		{
			...
			if(auto *pWinner = GameServer()->Bw().GetPlayer(m_Winner))
			{
				pWinner->Bw().AddExpMultiplier(Config()->m_SvZCatchGrenadeWinnerExpMultiplier, Config()->m_SvZCatchGrenadeWinnerExpDuration);
				pWinner->Bw().GiveFlag(Config()->m_SvZCatchGrenadeWinnerExpDuration);
				char aBonusBuf[256];
				...
				GameServer()->Bw().SendChatTarget(m_Winner, aBonusBuf);
			}
			// webhook
			{
	...
	{
		GameServer()->Bw().SendChatTarget(-1, "Not enough players joined zCatch Grenade.");
	}
	else if(m_FinishReason == EMERGENCY)
	{
		GameServer()->Bw().SendChatTarget(-1, "zCatch Grenade ended prematurely.");
	}
...
bool CZCatchGrenadeEvent::Register(int ClientId)
{
	...
	{
		GameServer()->Bw().SendChatTarget(ClientId, "You are already registered for zCatch Grenade.");
		return false;
	}
	...
	GameServer()->Bw().SendChatTarget(ClientId, "You joined zCatch Grenade registration!");
	return true;
}
...
bool CZCatchGrenadeEvent::DeRegister(int ClientId)
{
	...
	{
		GameServer()->Bw().SendChatTarget(ClientId, "You are not registered for zCatch Grenade.");
		return false;
	}
	m_Candidates.erase(it);
	GameServer()->Bw().SendChatTarget(ClientId, "You left zCatch Grenade registration.");
	return true;
}
...
void CZCatchGrenadeEvent::OnCharacterTakeDamage(vec2 Force, int Dmg, int From, int ClientId, int Weapon)
{
	...
	// if(GameServer()->Collision()->IntersectLine(SourcePos, pChar->m_Pos, nullptr, nullptr)) // BW: Source param dropped, was unused
	// 	return;
...
void CZCatchGrenadeEvent::HandleCamping(int ClientId)
{
	...
	{
		pPlayer->Bw().SendBroadcast("ANTICAMPER: Move or die");
	...
	if((m_CampTick[ClientId] <= Server()->Tick()) && (m_CampTick[ClientId] > 0))
	{
		...
		pPlayer->Bw().SendBroadcast("You have been frozen due to camping");
