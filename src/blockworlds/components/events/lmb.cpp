	...
	CEventComponent(pGameContext), m_DDRaceTeam(-1), m_Winner(-1), m_FinishingReason(NATURAL)
{
	...
	m_SpawnPositions = GameServer()->Bw().ZoneManager()->GetNamedQuadCenters("lmb_spawn");
...
void CLastManBlockingEvent::FinishEvent()
{
	...
	if(m_FinishingReason == NATURAL)
	{
		if(m_Winner != -1)
		{
			...
			CPlayer *pWinner = GameServer()->Bw().GetPlayer(m_Winner);
			if(pWinner)
			{
				if(pWinner->Bw().IsLoggedIn())
				{
					int BlockpointsReward = Config()->m_SvLMBBlockpointsReward;
					int PagesReward = Config()->m_SvLMBPagesReward;
					pWinner->Bw().SetPlayerBlockpoints(pWinner->Bw().GetPlayerBlockpoints() + BlockpointsReward);
					pWinner->Bw().SetPlayerPages(pWinner->Bw().GetPlayerPages() + PagesReward);
					pWinner->Bw().SetPlayerTourneyWins(pWinner->Bw().GetPlayerTourneyWin() + 1);
					...
					pWinner->Bw().AddExpMultiplier(Config()->m_SvLMBWinnerExpMultiplier, Config()->m_SvLMBWinnerExpMultiplierDuration);
					...
					GameServer()->Bw().SendChatTarget(m_Winner, aBuf);
					pWinner->Bw().GiveFlag(Config()->m_SvLMBWinnerExpMultiplierDuration);
				}
				else
				{
...
bool CLastManBlockingEvent::Join(int ClientId)
{
	...
	GameServer()->Bw().SendBroadcast(" ", ClientId);
	return true;
}
bool CLastManBlockingEvent::Leave(int ClientId)
{
	...
	GameServer()->Bw().SendChatTarget(ClientId, s_randomStrings[Index]);
	return true;
}
