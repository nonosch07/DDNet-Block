...
void CZCatchEvent::OnCharacterSpawn(int ClientId, vec2 /*SpawnPos*/)
{
	...
	if(IsParticipant(ClientId))
	{
		// free participant respawned so put them back in the arena
		auto *pChar = GameServer()->GetPlayerChar(ClientId);
...
vec2 CZCatchEvent::NextSpawnPos()
{
	if(m_SpawnPositions.empty())
		return {0, 0};
	vec2 pos = m_SpawnPositions[m_SpawnOffset % (int)m_SpawnPositions.size()];
	m_SpawnOffset = (m_SpawnOffset + 1) % (int)m_SpawnPositions.size();
	return pos;
}
