	...
	CEventComponent(pGameServer)
{
	...
	m_SpawnPositions = GameServer()->Bw().ZoneManager()->GetNamedQuadCenters("bbt_spawn");
...
void CBombTagEvent::StartEvent()
{
	...
	GameServer()->Bw().SendChatTarget(-1, "BombTag started ! Last man standing wins !");
...
void CBombTagEvent::FinishEvent()
{
	...
	if(m_FinishReason == NATURAL)
	{
		if(m_Winner != -1)
		{
			...
			{
				pWinner->Bw().AddExpMultiplier(Config()->m_SvBombTagWinnerExpMultiplier, Config()->m_SvBombTagWinnerExpDuration);
				pWinner->Bw().GiveFlag(Config()->m_SvBombTagWinnerExpDuration);
				char aBonusBuf[256];
	...
	{
		GameServer()->Bw().SendChatTarget(-1, "Not enough players joined BombTag.");
	}
	else if(m_FinishReason == EMERGENCY)
	{
		GameServer()->Bw().SendChatTarget(-1, "BombTag ended prematurely.");
	}
...
bool CBombTagEvent::Register(int ClientId)
{
	...
	{
		GameServer()->Bw().SendChatTarget(ClientId, "You are already registered for BombTag.");
		return false;
	}
	...
	GameServer()->Bw().SendChatTarget(ClientId, "You joined BombTag registration!");
	return true;
}
...
bool CBombTagEvent::Join(int ClientId)
{
	...
	if(pChar)
	{
		...
		pChar->ResetVelocity();
		pChar->Bw().FreezeForce(Config()->m_SvBombTagInitialFreezeTime);
...
void CBombTagEvent::OnCharacterTakeDamage(vec2 Force, int Dmg, int From, int ClientId, int Weapon)
{
	...
	else
	{
		if(ClientId == m_Bomb)
			...
			pChar->Bw().FreezeForce(g_BwConfig.m_SvBombTagStunDuration);
	}
}
...
void CBombTagEvent::OnCharacterSpawn(int ClientId, vec2 /*SpawnPos*/)
{
	if(GetState() == CEventComponent::EEventState::Active)
	{
		...
		{
			Leave(ClientId);
			GameServer()->Bw().SendChatTarget(ClientId, "You were eliminated!");
			GameServer()->Bw().SendBroadcast("You were eliminated!", ClientId);
		}
	}
}
...
void CBombTagEvent::OnTick()
{
	...
	if(GetState() == EEventState::Registration)
	{
		...
		for(int i = 0; i < Server()->MaxClients(); ++i)
		{
			...
			pPlayer->Bw().SendBroadcastAlignedLeft("BombTag is about to start!\n"
							       "Register with /join\n"
							       "Time left: %s\n\n"
							       "Participants: %" PRIzu "\n"
							       "%s",
	...
	else if(GetState() == EEventState::Active)
	{
		...
		{
			CPlayer *pPlayer = GameServer()->Bw().GetPlayer(m_Bomb);
