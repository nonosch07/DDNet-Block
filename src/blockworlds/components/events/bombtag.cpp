	...
	CEventComponent(pGameServer)
{
	...
	m_SpawnPositions = GameServer()->ZoneManager()->GetNamedQuadCenters("bbt_spawn");
...
void CBombTagEvent::StartEvent()
{
	...
	GameServer()->SendChatTarget(-1, "BombTag started ! Last man standing wins !");
...
void CBombTagEvent::FinishEvent()
{
	...
	if(m_FinishReason == NATURAL)
	{
		if(m_Winner != -1)
		{
			...
			{
				pWinner->AddExpMultiplier(Config()->m_SvBombTagWinnerExpMultiplier, Config()->m_SvBombTagWinnerExpDuration);
				pWinner->GiveFlag(Config()->m_SvBombTagWinnerExpDuration);
				char aBonusBuf[256];
	...
	{
		GameServer()->SendChatTarget(-1, "Not enough players joined BombTag.");
	}
	else if(m_FinishReason == EMERGENCY)
	{
		GameServer()->SendChatTarget(-1, "BombTag ended prematurely.");
	}
...
bool CBombTagEvent::Register(int ClientId)
{
	...
	{
		GameServer()->SendChatTarget(ClientId, "You are already registered for BombTag.");
		return false;
	}
	...
	GameServer()->SendChatTarget(ClientId, "You joined BombTag registration!");
	return true;
}
...
bool CBombTagEvent::DeRegister(int ClientId)
{
	...
	{
		GameServer()->SendChatTarget(ClientId, "You are not registered for BombTag.");
		return false;
	}
	m_Candidates.erase(it);
	GameServer()->SendChatTarget(ClientId, "You left BombTag registration.");
	return true;
}
...
bool CBombTagEvent::Join(int ClientId)
{
	...
	if(pChar)
	{
		...
		pChar->ResetVelocity();
		pChar->FreezeForce(Config()->m_SvBombTagInitialFreezeTime);
...
void CBombTagEvent::OnCharacterTakeDamage(vec2 Force, vec2 Source, int Dmg, int From, int ClientId, int Weapon)
{
	...
	else
	{
		if(ClientId == m_Bomb)
			...
			pChar->FreezeForce(Config()->m_SvBombTagStunDuration);
	}
}
...
void CBombTagEvent::OnSnapClientInfo(int ClientId, int SnappingClient, struct CNetObj_ClientInfo *pClientInfo)
{
	...
	StrToInts(&pClientInfo->m_Skin0, 6, ClientId == m_Bomb ? "bomb" : "default");
...
void CBombTagEvent::OnTick()
{
	...
	if(GetState() == EEventState::Registration)
	{
		...
		for(int i = 0; i < Server()->MaxClients(); ++i)
		{
			...
			pPlayer->SendBroadcastAlignedLeft("BombTag is about to start!\n"
							  "Register with /join\n"
							  "Time left: %s\n\n"
							  "Participants: %" PRIzu "\n"
							  "%s",
	...
	else if(GetState() == EEventState::Active)
	{
		...
		{
			CPlayer *pPlayer = GameServer()->GetPlayer(m_Bomb);
