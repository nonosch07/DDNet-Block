...
void CAiBotComponent::LoginBotAccount()
{
	...
	if(pBot->Bw().IsLoggedIn())
		return;
	pBot->Bw().SetPlayerId(0);
	pBot->Bw().SetPlayerName(m_aAccountName);
	pBot->Bw().SetPlayerPassword(m_aAccountPassword);
	// real registration/login path would create DB entries; here we just mark logged-in state mimic by giving Id != 0.
	pBot->Bw().SetPlayerId(13371337);
	pBot->Bw().m_IsNpc = true;
}
