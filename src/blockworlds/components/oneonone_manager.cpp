...
{
	if(!GameServer()->Bw().GetPlayer(Player1ID) || !GameServer()->Bw().GetPlayer(Player2ID))
	{
...
void COneOnOneManager::LeaveEventRegistration(int ClientId)
{
	...
	GameServer()->Bw().SendChatTarget(ClientId, "You were taken out of the event queue because your 1on1 is starting.");
}
		...
		if(m && (m->GetState() == COneOnOneEvent::EEventState::Active || m->GetState() == COneOnOneEvent::EEventState::Preparation))
		{
			...
			if(std::find(parts.begin(), parts.end(), ClientId) != parts.end())
			{
				if(m->GetState() == COneOnOneEvent::EEventState::Preparation)
				{
					...
					GameServer()->Bw().SendChatTarget(otherCid, "[1on1] Opponent disconnected during warmup. Match cancelled.");
