...
// Everything Blockworlds hangs off the game context.
//
// CGameContext owns one CBlockworlds and forwards a handful of lifecycle hooks
// to it; all BW subsystems, chat commands and per-round state live here instead
// of being spliced into upstream's gamecontext.{h,cpp}.
class CBlockworlds
{
public:
	explicit CBlockworlds(CGameContext *pGameServer);
	~CBlockworlds();
	...
	// indicator. Both return true when BW took over.
	...
	// has no predicate overload, so BW does its own search over the characters.
	...
	// Upstream's CGameContext::SnapLaserObject hardcodes m_Flags to 0. BW's
	// cosmetic lasers must be sent with LASERFLAG_NO_PREDICT, so BW snaps its own.
	...
	// --- broadcast helpers ---
	// BW code calls both shapes: upstream's (text, client, important) and BW's
	...
	// and remove commands while the server runs, so BW pushes the deltas itself.
	...
	// BW used to add a STATE_NPC client state to the server. Upstream has since
...
#endif // BLOCKWORLDS_BW_CONTEXT_H
