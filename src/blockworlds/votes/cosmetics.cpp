...
static std::string NormalizeForCompare(const std::string &s){
    ...
    for(size_t i=0;i<s.size();++i){
        unsigned char c = s[i];
        if(c < 0x80){
            if(std::isalnum(c) || std::isspace(c)) tmp.push_back(c);
        }
    }
    ...
    for(char ch : tmp){
        ...
        } else { out.push_back(ch); in_space=false; }
    }
...
CosmeticsVoteManager::CosmeticsVoteManager() : m_CategoriesInitialized(false) {}
...
void CosmeticsVoteManager::CreateStripline(char* pDst, int DstSize, const char* pTitle)
{
    ...
    std::string s; s.reserve(DstSize);
    for(int i=0;i<StripSideLen;++i) s.push_back('#');
    s += ' ';
    s += title;
    s += ' ';
    for(int i=0;i<StripSideLen;++i) s.push_back('#');
...
void CosmeticsVoteManager::SendOptions(CPlayer* pPlayer, int ClientID, IServer* pServer, CGameContext* pGameContext)
{
    ...
    if(pGameContext && pGameContext->m_pVoteOptionFirst) m_Options.push_back(std::string(" "));
    ...
    for(const auto& cat : m_Categories)
    {
        ...
        for(int i=0;i<cat.NumItems;i++){
            if(owned && owned[i]=='1'){
                ...
                if(norm.empty() || seenNormalized.find(norm)!=seenNormalized.end()) continue;
        ...
        if(!lines.empty()){
            ...
            for(const auto& s:lines) m_Options.push_back(s);
        }
    }
    ...
    while(OptionsSent < TotalOptions)
    {
        ...
        OptionMsg.m_pDescription0 = ""; OptionMsg.m_pDescription1 = ""; OptionMsg.m_pDescription2 = "";
        OptionMsg.m_pDescription3 = ""; OptionMsg.m_pDescription4 = ""; OptionMsg.m_pDescription5 = "";
        OptionMsg.m_pDescription6 = ""; OptionMsg.m_pDescription7 = ""; OptionMsg.m_pDescription8 = "";
        OptionMsg.m_pDescription9 = ""; OptionMsg.m_pDescription10 = ""; OptionMsg.m_pDescription11 = "";
        OptionMsg.m_pDescription12 = ""; OptionMsg.m_pDescription13 = ""; OptionMsg.m_pDescription14 = "";
...
{
    if(!pPlayer || !pGameContext) return false;
    EnsureCategoriesInitialized();
    ...
    for(const auto& cat : m_Categories){
        for(int i=0;i<cat.NumItems;++i){
            ...
            if(normalizedVote.find(normalizedName) != std::string::npos){
                ...
                if(headerStr == "Skin Manipulations") toggled = pGameContext->Cosmetics()->ToggleSkinmani(ClientId, Name.c_str());
                else if(headerStr == "Gun Designs") toggled = pGameContext->Cosmetics()->ToggleGundesign(ClientId, Name.c_str());
                else if(headerStr == "Knockout Effects") toggled = pGameContext->Cosmetics()->ToggleKnockout(ClientId, Name.c_str());
                else toggled = pGameContext->Cosmetics()->ToggleGundesign(ClientId, Name.c_str()) || pGameContext->Cosmetics()->ToggleKnockout(ClientId, Name.c_str()) || pGameContext->Cosmetics()->ToggleSkinmani(ClientId, Name.c_str());
                if(toggled){ pGameContext->ClearVotes(ClientId); return true; }
