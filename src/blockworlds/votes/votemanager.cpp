		...
		if(E.first == VoteInput)
		{
			...
			case EActionKind::RedirectToPort:
				if(pGameContext && pGameContext->Server())
				{
					...
					if(DoRedirect)
						pGameContext->Bw().RedirectClient(ClientId, Port);
				}
				return true;
			...
			case EActionKind::OpenLeaderboardCategory:
				if(pPlayer)
				{
					pPlayer->Bw().m_CaptureTopToMenu = true;
					pPlayer->Bw().m_CaptureTopCategory = A.m_A; // 0..3
					pPlayer->Bw().m_TopMessagesCount = 0;
				}
				...
				if(pGameContext)
				{
					if(A.m_A == 0 && pGameContext->Bw().Accounts())
						pGameContext->Bw().Accounts()->ShowTopLevel(ClientId);
					else if(A.m_A == 1 && pGameContext->Bw().Accounts())
						pGameContext->Bw().Accounts()->ShowTopBlockpoints(ClientId);
					else if(A.m_A == 2 && pGameContext->Bw().Accounts())
						pGameContext->Bw().Accounts()->ShowTopKillStreak(ClientId);
					else if(A.m_A == 3 && pGameContext->Bw().Clans())
						pGameContext->Bw().Clans()->ShowTopClans(ClientId);
				}
				return true;
			...
			case EActionKind::TogglePassive:
				if(pPlayer)
				{
					pPlayer->Bw().TogglePassive();
					bool EnabledForMessage = pPlayer->Bw().IsUsingPassiveProtection() || pPlayer->Bw().IsPassivePendingEnable();
			...
			case EActionKind::ToggleCosmeticItem:
				if(pGameContext)
				{
					...
					if(Cat == 0) // Skin Manipulations
						Changed = pGameContext->Bw().Cosmetics()->ToggleSkinmani(ClientId, CCosmeticsHandler::ms_SkinmaniNames[Item]);
					else if(Cat == 1) // Gun Designs
						Changed = pGameContext->Bw().Cosmetics()->ToggleGundesign(ClientId, CCosmeticsHandler::ms_GundesignNames[Item]);
					else if(Cat == 2) // Knockout Effects
						Changed = pGameContext->Bw().Cosmetics()->ToggleKnockout(ClientId, CCosmeticsHandler::ms_KnockoutNames[Item]);
					else if(Cat == 3) // VIP Specials
					{
						...
						Changed = pGameContext->Bw().Cosmetics()->ToggleSpecial(ClientId, s_VipNames[Item]);
					}
					if(Changed)
					{
						pGameContext->Bw().ClearVotes(ClientId);
			...
			case EActionKind::ToggleHideCosmetics:
				if(pPlayer)
				{
					pPlayer->Bw().m_HideCosmetics = !pPlayer->Bw().m_HideCosmetics;
					pGameContext->SendChatTarget(ClientId, pPlayer->Bw().m_HideCosmetics ? "Cosmetics hidden." : "Cosmetics visible.");
					pGameContext->Bw().ClearVotes(ClientId);
					if(IsAtRoot(ClientId))
						pGameContext->ProgressVoteOptions(ClientId, true);
					RenderCurrentPage(pPlayer, ClientId, pGameContext->Server(), pGameContext);
			...
			case EActionKind::SetScoreMode:
				if(pPlayer)
				{
					pPlayer->Bw().m_ScoreDisplayMode = A.m_A;
			...
			case EActionKind::DuelSetPoints:
			{
				...
				if(Match && Match->IsInConfigPhase())
				{
					...
					{
						CPlayer *pP = pGameContext->Bw().GetPlayer(Cid);
						if(pP)
						{
							pGameContext->Bw().ClearVotes(Cid);
...
void CVoteManager::RenderCurrentPage(CPlayer *pPlayer, int ClientID, IServer *pServer, CGameContext *pGameContext)
{
	...
	// beautify: wrap page with box-drawing header/footer
	{
		...
		switch(Current.m_PageType)
		{
		case SPage::ROOT: Title = "Blockworlds Menu"; break;
...
void CVoteManager::BuildRoot(CPlayer *pPlayer, int ClientID, IServer *pServer, CGameContext *pGameContext, std::vector<std::string> &OutLabels, std::vector<SAction> &OutActions)
{
	if(pPlayer)
	{
		bool HasPassive = (pPlayer->Bw().m_LocalPassiveDuration > 0) || (pPlayer->Bw().IsLoggedIn() && pPlayer->Bw().GetPlayerPassive() > 0);
		if(HasPassive)
		{
			std::string PassiveLine;
			if(pPlayer->Bw().IsUsingPassiveProtection() || pPlayer->Bw().IsPassivePendingEnable())
	...
	if(pPlayer)
	{
		int Mode = pPlayer->Bw().m_ScoreDisplayMode;
		if(Mode < 0 || Mode > 1)
			Mode = 0;
...
void CVoteManager::BuildLeaderboardDetail(CPlayer *pPlayer, int ClientID, IServer *pServer, CGameContext *pGameContext, int CategoryIndex, std::vector<std::string> &OutLabels, std::vector<SAction> &OutActions)
{
	...
	if(pPlayer->Bw().m_TopMessagesCount <= 0 || pPlayer->Bw().m_CaptureTopCategory != CategoryIndex)
	{
		// spacer above inner box
		OutLabels.emplace_back("");
		OutActions.emplace_back(SAction{EActionKind::None});
	...
	for(int i = 0; i < pPlayer->Bw().m_TopMessagesCount; ++i)
	{
		const char *pLine = pPlayer->Bw().m_aTopMessages[i];
		if(!pLine || !pLine[0])
			break;
...
void CVoteManager::BuildCosmeticsRoot(CPlayer *pPlayer, int ClientID, IServer *pServer, CGameContext *pGameContext, std::vector<std::string> &OutLabels, std::vector<SAction> &OutActions)
{
	...
	RenderSection("Skin Manipulations",
		...
		0, pPlayer->Bw().GetSkinMani(),
		[&](int i) { return pGameContext->Bw().Cosmetics()->HasSkinmani(Cid, i); });

	// Gun Designs (category 1)
	RenderSection("Gun Designs",
		CCosmeticsHandler::ms_GundesignNames, CCosmeticsHandler::NUM_GUNDESIGNS,
		1, pPlayer->Bw().GetGunDesign(),
		[&](int i) { return pGameContext->Bw().Cosmetics()->HasGundesign(Cid, i); });
	...
	RenderSection("Knockout Effects",
		...
		2, pPlayer->Bw().GetKnockout(),
		[&](int i) { return pGameContext->Bw().Cosmetics()->HasKnockoutEffect(Cid, i); });

	// VIP Items (category 3) — only for VIP players
	if(pPlayer->Bw().GetPlayerVip() || (pServer && pServer->GetAuthedState(Cid) != AUTHED_NO))
	{
		static const char *s_Vip[] = {"Ball", "Crown", "Epic Circle", "Halo"};
		RenderSection("VIP Items",
			s_Vip, 4,
			3, pPlayer->Bw().GetCurrentSpecial(),
			[&](int i) { return pGameContext->Bw().Cosmetics()->HasSpecial(Cid, i); });
	}
...
void CVoteManager::BuildCosmeticsCategory(CPlayer *pPlayer, int ClientID, IServer *pServer, CGameContext *pGameContext, int CategoryIndex, std::vector<std::string> &OutLabels, std::vector<SAction> &OutActions)
{
	...
	if(!(pPlayer && pGameContext && pPlayer->Bw().IsLoggedIn()))
	{
		OutLabels.emplace_back(SmallCaps("/login to use cosmetics!"));
		OutActions.emplace_back(SAction{EActionKind::None});
		return;
	...
	else if(CategoryIndex == 3)
	{
		...
		if(!(pPlayer->Bw().GetPlayerVip() || (pServer && pServer->GetAuthedState(pPlayer->GetCid()) != AUTHED_NO)))
		{
	...
	int Active = -1;
	if(CategoryIndex == 0)
		Active = pPlayer->Bw().GetSkinMani();
	else if(CategoryIndex == 1)
		Active = pPlayer->Bw().GetGunDesign();
	else if(CategoryIndex == 2)
		Active = pPlayer->Bw().GetKnockout();
	else if(CategoryIndex == 3)
		Active = pPlayer->Bw().GetCurrentSpecial();
	...
	for(int i = 0; i < Count; ++i)
	{
		bool Has = false;
		if(CategoryIndex == 0)
			Has = pGameContext->Bw().Cosmetics()->HasSkinmani(pPlayer->GetCid(), i);
		else if(CategoryIndex == 1)
			Has = pGameContext->Bw().Cosmetics()->HasGundesign(pPlayer->GetCid(), i);
		else if(CategoryIndex == 2)
			Has = pGameContext->Bw().Cosmetics()->HasKnockoutEffect(pPlayer->GetCid(), i);
		else if(CategoryIndex == 3)
			Has = pGameContext->Bw().Cosmetics()->HasSpecial(pPlayer->GetCid(), i);
		if(Has)
			Owned.insert(i);
	}
...
void CVoteManager::BuildShop(CPlayer *pPlayer, int ClientID, IServer *pServer, CGameContext *pGameContext, std::vector<std::string> &OutLabels, std::vector<SAction> &OutActions)
{
	...
	{
		char aBuf[128];
		str_format(aBuf, sizeof(aBuf), "Your BP: %d", pPlayer->Bw().GetPlayerBlockpoints());
...
void CVoteManager::BuildShopCategory(CPlayer *pPlayer, int ClientID, IServer *pServer, CGameContext *pGameContext, int CategoryIndex, std::vector<std::string> &OutLabels, std::vector<SAction> &OutActions)
{
	...
	CCosmeticsHandler *pCos = pGameContext->Bw().Cosmetics();
	if(!pCos)
		return;
	...
	{
		char aBuf[128];
		str_format(aBuf, sizeof(aBuf), "Your BP: %d  |  Level: %d", pPlayer->Bw().GetPlayerBlockpoints(), pPlayer->Bw().GetPlayerLevel());
		OutLabels.emplace_back(aBuf);
		OutActions.emplace_back(SAction{EActionKind::None});
