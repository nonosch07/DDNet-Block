#!/usr/bin/env python3
"""List Blockworlds integration that upstream files are still missing.

The port moved BW's own code into src/blockworlds/, but BW also inserted code
*inside* upstream function bodies. This walks the old fork and reports, per
function, which of those insertions do not yet have a matching `BW BEGIN` block
...
WONTPORT = {
    ("src/game/server/gamecontext.cpp", "SnapLaserObject"): "BW snaps its own laser (CBlockworlds::SnapLaserObject)",
    ("src/game/server/gamecontext.cpp", "SnapPickup"): "upstream grew the Flags parameter itself",
    ("src/game/server/gamecontext.cpp", "RegisterChatCommands"): "BW commands are registered from CBlockworlds::OnConsoleInit",
    ("src/game/server/gamecontext.cpp", "RegisterDDRaceCommands"): "same",
    ("src/game/server/gamecontext.cpp", "Construct"): "ported as CBlockworlds::OnConstruct",
    ("src/game/server/gamecontext.cpp", "Destruct"): "ported as CBlockworlds::OnDestruct",
    ("src/game/server/gamecontroller.cpp", "OnPlayerDisconnect"): "ported into CGameControllerBW",
    ("src/game/server/gamecontroller.cpp", "Snap"): "ported as OnSnapGameInfo/OnSnapGameInfoEx",
    ("src/game/server/gamecontroller.cpp", "OnCharacterSpawn"): "ported into CGameControllerBW",
    ...
    ("src/game/server/entities/character.cpp", "Spawn"): "ported as CBlockworlds::OnCharacterSpawn, called from CGameControllerBW",
    ...
    ("src/game/server/ddracechat.cpp", "ConCredits"): "upstream removed /credits; BW registers its own ConCredits",
    ("src/game/server/gamecontroller.cpp", "Tick"): "ported as CGameControllerBW::Tick",
    ...
    ("src/engine/shared/network_server.cpp", "Send"): "BW bots use upstream's m_DebugDummy, which Send already skips",
    ...
    ("src/engine/server/server.cpp", "RedirectClient"): "ported as CBlockworlds::RedirectClient",
...
def hooked_functions(path, cls):
    """Function names in the current tree that contain a `BW BEGIN` marker."""
    try:
...
def main():
    ...
    ap.add_argument("-v", "--verbose", action="store_true", help="print the missing BW lines")
    args = ap.parse_args()
    ...
    for path, cls in FILES.items():
        ...
        print(f"[{status}] {path}")
        print(f"        {done}/{len(rows)} functions done, {missing} BW lines missing")
    ...
    print(f"== {todo_fns} functions still need a BW hook, {total_missing} lines ==")
