...
bool CWhoIs::GetClientIdentity(int ClientId, char *pOutIp, int OutIpSize, char *pOutName, int OutNameSize, int &OutAccId, char *pOutAccName, int OutAccNameSize)
{
	if(!Server()->ClientIngame(ClientId) || !GameServer()->m_apPlayers[ClientId])
		return false;
	BwClientAddr(Server(), ClientId, pOutIp, OutIpSize);
	...
	OutAccId = pPl->Bw().IsLoggedIn() ? pPl->Bw().m_Account.m_Id : 0;
	if(pPl->Bw().IsLoggedIn())
		str_copy(pOutAccName, pPl->Bw().m_Account.m_aName, OutAccNameSize);
	else
		pOutAccName[0] = '\0';
	return true;
}
...
void CWhoIs::LogLeave(int ClientId)
{
	...
	BwClientAddr(Server(), ClientId, aIp, sizeof(aIp));
	NormalizeIpNoPort(aIp);
	str_copy(aName, Server()->ClientName(ClientId), sizeof(aName));
	if(GameServer()->m_apPlayers[ClientId] && GameServer()->m_apPlayers[ClientId]->Bw().IsLoggedIn())
	{
		AccId = GameServer()->m_apPlayers[ClientId]->Bw().m_Account.m_Id;
		str_copy(aAcc, GameServer()->m_apPlayers[ClientId]->Bw().m_Account.m_aName, sizeof(aAcc));
	}
	auto pReq = std::make_unique<CSqlWhoIsLog>(nullptr);
	str_copy(pReq->m_aIp, aIp, sizeof(pReq->m_aIp));
	str_copy(pReq->m_aName, aName, sizeof(pReq->m_aName));
