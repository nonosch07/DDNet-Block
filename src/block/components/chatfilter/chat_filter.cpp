...
bool CChatFilterComponent::CheckAndMaybeMute(int ClientId, const char *pMessage)
{
	...
	if(FoundBadWord)
	{
		...
		if(Hours > 0)
		{
			...
			int Remaining = GameServer()->Block().GetRemainingMuteSecondsPublic(ClientId);

			GameServer()->Block().AddIpMuteSilent(&Addr, Secs, "Chat filter violation");
			...
			if(Remaining > 0)
			{
				GameServer()->Block().SendChatTarget(ClientId, "Your message was blocked by the chat filter. You are already muted.");
			}
			else
			{
				char aBuf[128];
		...
		{
			GameServer()->Block().SendChatTarget(ClientId, "Your message was blocked by the chat filter.");
		}
		return true;
	}
