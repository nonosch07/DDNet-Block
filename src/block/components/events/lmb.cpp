	...
	CEventComponent(pGameContext), m_DDRaceTeam(-1), m_Winner(-1), m_FinishingReason(NATURAL)
{
	...
	m_SpawnPositions = GameServer()->Block().ZoneManager()->GetNamedQuadCenters("lmb_spawn");
...
void CLastManBlockingEvent::OnTick()
{
	...
	else if(GetState() == CEventComponent::EEventState::Active)
	{
		...
		for(const auto &ClientId : Participants())
		{
			...
			pPlayer->Block().SendBroadcastAlignedLeft("Participants left: %" PRIzu "\n"
								  "Time left: %s",
				Participants().size(),
				aTimeLeft);
		}
...
void CLastManBlockingEvent::FinishEvent()
{
	...
	if(m_FinishingReason == NATURAL)
	{
		if(m_Winner != -1)
		{
			...
			CPlayer *pWinner = GameServer()->Block().GetPlayer(m_Winner);
			if(pWinner)
			{
				if(pWinner->Block().IsLoggedIn())
				{
					int BlockpointsReward = Config()->m_SvLMBBlockpointsReward;
					int PagesReward = Config()->m_SvLMBPagesReward;
					pWinner->Block().SetPlayerBlockpoints(pWinner->Block().GetPlayerBlockpoints() + BlockpointsReward);
					pWinner->Block().SetPlayerPages(pWinner->Block().GetPlayerPages() + PagesReward);
					pWinner->Block().SetPlayerTourneyWins(pWinner->Block().GetPlayerTourneyWin() + 1);
					str_format(aBuf, sizeof(aBuf), "You've received %d blockpoints and %d pages for winning!", BlockpointsReward, PagesReward);
					GameServer()->Block().SendChatTarget(m_Winner, aBuf);

					pWinner->Block().AddExpMultiplier(Config()->m_SvLMBWinnerExpMultiplier, Config()->m_SvLMBWinnerExpMultiplierDuration);
					...
					GameServer()->Block().SendChatTarget(m_Winner, aBuf);
					pWinner->Block().GiveFlag(Config()->m_SvLMBWinnerExpMultiplierDuration);
				}
				else
				{
					GameServer()->Block().SendChatTarget(m_Winner, "You must be logged in to receive rewards.");
				}
			}
		}
		else
	...
	for(const auto &SoloEntry : m_PrevSoloState)
	{
		if(auto *pChar = GameServer()->GetPlayerChar(SoloEntry.first))
		{
			if(SoloEntry.second.m_Solo)
				pChar->SetSolo(true);
			pChar->Block().Core().m_CollisionDisabled = SoloEntry.second.m_Collision;
		}
	}
...
bool CLastManBlockingEvent::Join(int ClientId)
{
	...
	{
		pChar->ResetVelocity();
		pChar->Block().FreezeForce(Config()->m_SvLMBInitialFreezeTime);
		m_FrozenSince.emplace(ClientId, Server()->Tick());
		GameServer()->Block().Teleport(pChar, RandomSpawnPos(m_SpawnPositions, m_UsedSpawnIndices));
	}
	...
	GameServer()->Block().SendBroadcast(" ", ClientId);
	return true;
}
bool CLastManBlockingEvent::Leave(int ClientId)
{
	...
	if(auto *pChar = GameServer()->GetPlayerChar(ClientId))
	{
		...
		if(It != m_PrevSoloState.end())
		{
			if(It->second.m_Solo)
				pChar->SetSolo(true);
			pChar->Block().Core().m_CollisionDisabled = It->second.m_Collision;
	...
	GameServer()->Block().SendChatTarget(ClientId, s_randomStrings[Index]);
	return true;
}
