...
{
	m_SpawnPositions = GameServer()->Block().ZoneManager()->GetNamedQuadCenters("zcb_spawn");
...
void CZCatchEvent::FinishEvent()
{
	...
	{
		GameServer()->Block().SendChatTarget(-1, "Not enough players joined zCatch.");
	}
	else if(m_FinishReason == EMERGENCY)
	{
		GameServer()->Block().SendChatTarget(-1, "zCatch ended prematurely.");
	}
...
bool CZCatchEvent::Register(int ClientId)
{
	...
	{
		GameServer()->Block().SendChatTarget(ClientId, "You are already registered for zCatch.");
		return false;
	}
	...
	GameServer()->Block().SendChatTarget(ClientId, "You joined zCatch registration!");
	return true;
}
...
bool CZCatchEvent::DeRegister(int ClientId)
{
	...
	{
		GameServer()->Block().SendChatTarget(ClientId, "You are not registered for zCatch.");
		return false;
	}
	m_Candidates.erase(It);
	GameServer()->Block().SendChatTarget(ClientId, "You left zCatch registration.");
	return true;
}
...
bool CZCatchEvent::AllowKillCommandFor(int ClientId) const
{
	...
	int Impactor = GameServer()->Block().BlockTracker().GetImpactorOf(ClientId);
...
void CZCatchEvent::OnTick()
{
	...
	if(GetState() == EEventState::Registration)
	{
		...
		for(int i = 0; i < Server()->MaxClients(); ++i)
		{
			...
			pPlayer->Block().SendBroadcastAlignedLeft("zCatch is about to start!\n"
								  "Register with /join\n"
								  "Time left: %s\n\n"
								  "Participants: %" PRIzu "\n"
								  "%s",
				...
				for(int Id : m_Participants)
				{
					...
					else if(IsFrozen && FsIt != m_FrozenSince.end())
					{
						...
						if(FrozenTicks >= LimitTicks)
						{
							...
							int Impactor = GameServer()->Block().BlockTracker().GetImpactorOf(Id);
							if(Impactor >= 0 && IsParticipant(Impactor) && !IsCaught(Impactor))
								ToKill.push_back(Id);
