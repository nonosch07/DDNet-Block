	...
	CEventComponent(pGameContext), m_Rng((unsigned)std::random_device{}() ^ (unsigned)pGameContext->Server()->Tick())
{
	...
	m_EventStartPositions = pGameContext->Block().ZoneManager()->GetNamedQuadCenters("tdm_spawn");
}
...
{
	if(auto *p = GameServer()->Block().GetPlayer(ClientId))
	{
		p->Block().m_OldTeeInfos = p->TeeInfos();
		...
		p->Block().m_HideInfo = true;
		p->Block().m_HideInfoInScoreboard = true;

		// strip cosmetics
		p->Block().ClearCosmetics();
		p->Block().SetSkinMani(-1);
		p->Block().SetGunDesign(-1);
		p->Block().SetKnockout(-1);
		if(p->Block().GetCurrentSpecial() != -1)
			p->Block().ToggleSpecial(p->Block().GetCurrentSpecial());
	}
}
...
void CTeamDeathmatchEvent::RestoreParticipantVisuals(int ClientId)
{
	if(auto *p = GameServer()->Block().GetPlayer(ClientId))
	{
		p->SetTeeInfos(p->Block().m_OldTeeInfos);
		p->Block().m_HideInfo = false;
		p->Block().m_HideInfoInScoreboard = false;
	}
}
...
void CTeamDeathmatchEvent::RestoreParticipants()
{
	...
	for(const auto &Kv : m_PrevSoloState)
	{
		if(auto *pChar = GameServer()->GetPlayerChar(Kv.first))
		{
			if(Kv.second.m_Solo)
				pChar->SetSolo(true);
			pChar->Block().Core().m_CollisionDisabled = Kv.second.m_Collision;
		}
	}
...
void CTeamDeathmatchEvent::TeleportToSpawn(int ClientId)
{
	if(auto *pChar = GameServer()->GetPlayerChar(ClientId))
	{
		if(auto Pos = ChooseSpawnFor(ClientId))
			GameServer()->Block().Teleport(pChar, *Pos);
		pChar->ResetVelocity();
	}
}
...
void CTeamDeathmatchEvent::BroadcastStatus()
{
	...
	for(int ClientId : m_Participants)
	{
		...
		if(Side == 0)
		{
			pPlayer->Block().SendBroadcastAlignedLeft("Team Blue\n"
								  "Blue %d / %d\n"
								  "Red %d / %d\n"
								  "Time left: %s",
		...
		{
			pPlayer->Block().SendBroadcastAlignedLeft("Team Red\n"
								  "Red %d / %d\n"
								  "Blue %d / %d\n"
								  "Time left: %s",
		...
		{
			pPlayer->Block().SendBroadcastAlignedLeft("%s\n"
								  "Blue %d / %d\n"
								  "Red %d / %d\n"
								  "Time left: %s",
...
void CTeamDeathmatchEvent::ApplyGroundHookPenalty(int ClientId)
{
	...
	{
		if(auto *pChar = GameServer()->GetPlayerChar(ClientId))
			pChar->Block().FreezeForce(Config()->m_SvGroundHookPenalty);
	}
}
...
void CTeamDeathmatchEvent::UpdatePerPlayerScores()
{
	...
	{
		if(auto *p = GameServer()->Block().GetPlayer(Pid))
		{
			int Side = GetSideOf(Pid);
			p->Block().m_Score = Side == 0 ? m_ScoreTeam[0] : m_ScoreTeam[1];
		}
	}
}
...
void CTeamDeathmatchEvent::UpdateRespawns()
{
	...
	for(auto &Kv : m_RespawnAtTick)
	{
		...
		if(SecsLeft != LastShown)
		{
			...
			if(SecsLeft > 0)
			{
				CPlayer *pPlayer = GameServer()->Block().GetPlayer(Cid);
				if(pPlayer)
					pPlayer->Block().SendBroadcastAlignedLeft("You will respawn in %d %s", SecsLeft, SecsLeft == 1 ? "sec" : "secs");
			}
		}
		if(Server()->Tick() >= When)
			Ready.push_back(Cid);
	...
	for(int Cid : Ready)
	{
		...
		{
			if(CPlayer *p = GameServer()->Block().GetPlayer(Cid))
			{
				...
				GameServer()->Block().SendBroadcast(" ", Cid, false); // clear countdown
			}
		}
	}
}
...
void CTeamDeathmatchEvent::UpdateSetSpectators()
{
	...
	for(const auto &Kv : m_SetSpecAtTick)
	{
		...
		if(Server()->Tick() < When)
			continue;
		CPlayer *p = GameServer()->Block().GetPlayer(Cid);
		if(!p)
		{
			Done.push_back(Cid);
			continue;
...
void CTeamDeathmatchEvent::OnTick()
{
	...
	if(GetState() == CEventComponent::EEventState::Registration)
	{
		...
		for(int i = 0; i < Server()->MaxClients(); ++i)
		{
			...
			pPlayer->Block().SendBroadcastAlignedLeft("%s is about to start!\nRegister with /join\nTime left: %s\n\nParticipants: %zd",
	...
	else if(GetState() == CEventComponent::EEventState::Active)
	{
		for(int Id : Participants())
		{
			...
			if(auto *p = GameServer()->Block().GetPlayer(Id))
			{
				p->Block().m_HideInfo = true;
				p->Block().m_HideInfoInScoreboard = true;
			}
...
void CTeamDeathmatchEvent::OnCharacterSpawn(int ClientId, vec2 /*SpawnPos*/)
{
	...
	if(auto *pChar = GameServer()->GetPlayerChar(ClientId))
	{
		if(m_ActiveStartTick != -1 && Server()->Tick() < m_ActiveStartTick + Config()->m_SvTDMFreezeTime * Server()->TickSpeed())
		{
			pChar->Block().FreezeForce(Config()->m_SvTDMFreezeTime);
			SetFrozenSince(ClientId, Server()->Tick());
		}
	}
}
...
void CTeamDeathmatchEvent::CloseRegistration()
{
	...
	GameServer()->Block().SendBroadcast(-1, " ", false); // clear registration broadcast for all

	if((int)m_Candidates.size() < Config()->m_SvTDMMinimumCandidates)
	{
		GameServer()->Block().SendChatTarget(-1, "Not enough candidates joined %s", GetEventName());
		GameServer()->Block().SendBroadcast(-1, "Not enough candidates joined %s", GetEventName());
...
void CTeamDeathmatchEvent::FinishEvent()
{
	...
	if(m_ScoreTeam[0] > m_ScoreTeam[1])
		GameServer()->Block().SendBroadcast(-1, "Team Blue wins %d - %d", m_ScoreTeam[0], m_ScoreTeam[1]);
	else if(m_ScoreTeam[1] > m_ScoreTeam[0])
		GameServer()->Block().SendBroadcast(-1, "Team Red wins %d - %d", m_ScoreTeam[1], m_ScoreTeam[0]);
	else
		GameServer()->Block().SendBroadcast(-1, "TDM ended in a tie %d - %d", m_ScoreTeam[0], m_ScoreTeam[1]);
	...
	auto Award = [&](const std::vector<SRanked> &Team, bool IsWinner) {
		for(int i = 0; i < 3 && i < (int)Team.size(); ++i)
		{
			...
			if(Bp > 0)
			{
				if(auto *pPlayer = GameServer()->Block().GetPlayer(Team[i].m_ClientId))
				{
					if(!pPlayer->Block().IsLoggedIn())
					{
						GameServer()->Block().SendChatTarget(Team[i].m_ClientId, "You must be logged in to receive rewards.");
						continue;
					}
					pPlayer->Block().SetPlayerBlockpoints(pPlayer->Block().GetPlayerBlockpoints() + Bp);
					GameServer()->Block().Accounts()->Save(Team[i].m_ClientId, &pPlayer->Block().m_Account);
					char aBuf[128];
					str_format(aBuf, sizeof(aBuf), "You received %d BP for your performance in TDM.", Bp);
					GameServer()->Block().SendChatTarget(Team[i].m_ClientId, aBuf);
				}
			}
		}
	};
...
bool CTeamDeathmatchEvent::Register(int ClientId)
{
	...
	if(IsCandidate(ClientId))
	{
		GameServer()->Block().SendChatTarget(ClientId, "You already registered to participate.");
		return false;
	}
	...
	if((int)m_Candidates.size() >= Config()->m_SvTDMMaximumCandidates)
	{
		GameServer()->Block().SendChatTarget(ClientId, "Registration is full (max %d players).", Config()->m_SvTDMMaximumCandidates);
		return false;
	}

	m_Candidates.push_back(ClientId);
	GameServer()->Block().SendChatTarget(ClientId, "You successfully joined %s!", GetEventName());
	return true;
}
...
bool CTeamDeathmatchEvent::DeRegister(int ClientId)
{
	...
	{
		GameServer()->Block().SendChatTarget(ClientId, "You aren't registered to participate.");
		return false;
	}
	m_Candidates.erase(It);
	GameServer()->Block().SendChatTarget(ClientId, "You successfully left %s.", GetEventName());
	return true;
}
	...
	if(GetState() == CEventComponent::EEventState::Active)
	{
		...
		if(Penalty > 0)
		{
			if(auto *pPlayer = GameServer()->Block().GetPlayer(ClientId))
			{
				if(pPlayer->Block().IsLoggedIn())
				{
					int Bp = pPlayer->Block().GetPlayerBlockpoints();
					pPlayer->Block().SetPlayerBlockpoints(std::max(0, Bp - Penalty));
					GameServer()->Block().Accounts()->Save(ClientId, &pPlayer->Block().m_Account);
					char aBuf[128];
	...
	if(auto ItSolo = m_PrevSoloState.find(ClientId); ItSolo != m_PrevSoloState.end())
	{
		if(auto *pChar = GameServer()->GetPlayerChar(ClientId))
		{
			if(ItSolo->second.m_Solo)
				pChar->SetSolo(true);
			pChar->Block().Core().m_CollisionDisabled = ItSolo->second.m_Collision;
		}
