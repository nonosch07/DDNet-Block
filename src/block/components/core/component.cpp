...
void CComponent::OnConsoleInit()
{
	...
	GameServer()->Block().SendChatCmdGroupStart(-1);
		...
		for(int i = 0; i < MAX_CLIENTS; ++i)
		{
			...
			if(pServer->CanClientUseCommand(i, Command))
			{
				...
				if(Command->Flags() & CFGFLAG_CHAT)
					GameServer()->Block().SendChatCmdAdd(Command, i);
			}
		}
	}
	GameServer()->Block().SendChatCmdGroupEnd(-1);
...
void CComponent::OnConsoleTerminate()
{
	for(auto &Cmd : m_ConsoleCommands)
	{
		...
		if(Command)
		{
			...
			if(Command->Flags() & CFGFLAG_CHAT)
				GameServer()->Block().SendChatCmdRem(Command, -1);
		}
