...
#include <block/components/agones/agones.h>
#include <block/components/ai/ai_bot.h>
#include <block/components/chatfilter/chat_filter.h>
#include <block/components/clientdetect/client_detect.h>
...
#include <block/components/promises.h>
#include <block/components/requests.h>
#include <block/components/vpndetection/vpn_detection.h>
...
#include <block/zones/passivezone.h>

#include <algorithm>

CBlock::CBlock(CGameContext *pGameServer) :
...
IServer *CBlock::Server() const { return m_pGameServer->Server(); }
IConsole *CBlock::Console() const { return m_pGameServer->Console(); }
IEngine *CBlock::Engine() const { return m_pGameServer->Engine(); }
void CBlock::Teleport(CCharacter *pChr, vec2 Pos) { m_pGameServer->Teleport(pChr, Pos); }
...
/// The components the snapshot path asks for, looked up once a tick.
///
/// SnapPlayerScore and OnSnapPlayerInfo run once per player per viewer: on a
/// full server that is 128 x 128 x 50 calls a second, and each one was doing a
/// type_index hash lookup plus a shared_ptr copy to discover, almost always,
/// that no event is running. The lookups happen once per tick now instead.
///
/// Ownership is kept, not just the address: component_unplug runs from a console
/// command, which can land between the refresh and the snapshot, and a raw
/// pointer would dangle for the rest of the tick.
namespace
{
	struct SSnapComponentCache
	{
		CComponentAccessor<COneOnOneManager> m_OneOnOne{nullptr};
		CComponentAccessor<CEvents> m_Events{nullptr};

		void Refresh()
		{
			m_OneOnOne = g_ComponentRegistry.Get<COneOnOneManager>();
			m_Events = g_ComponentRegistry.Get<CEvents>();
		}
		void Clear()
		{
			m_OneOnOne = nullptr;
	...
	SSnapComponentCache g_SnapCache;
} // namespace

void CBlock::BLOCK_OnTick()
{
	g_SnapCache.Refresh();
...
void CBlock::ClearVotes(int ClientID) const
{
...
void CBlock::ConSendSound(IConsole::IResult *pResult, void *pUserData)
{
...
void CBlock::ConSetWeaponkits(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	if(pResult->NumArguments() == 0)
	{
		pSelf->SendChat(-1, TEAM_ALL, pSelf->Block().m_WeaponkitsAllowed ? "Weaponkits allowed: true" : "Weaponkits allowed: false");
		return;
	}
	const char *pArg = pResult->GetString(0);
	if(str_comp(pArg, "1") == 0 || str_comp_nocase(pArg, "on") == 0 || str_comp_nocase(pArg, "true") == 0)
		pSelf->Block().m_WeaponkitsAllowed = true;
	else if(str_comp(pArg, "0") == 0 || str_comp_nocase(pArg, "off") == 0 || str_comp_nocase(pArg, "false") == 0)
		pSelf->Block().m_WeaponkitsAllowed = false;
	else
	{
	...
	g_Config.m_SvWeaponkitsAllowed = pSelf->Block().m_WeaponkitsAllowed ? 1 : 0;
	if(pSelf->ConfigManager())
		pSelf->ConfigManager()->Save();

	pSelf->SendChat(-1, TEAM_ALL, pSelf->Block().m_WeaponkitsAllowed ? "Weaponkits are now allowed on this server." : "Weaponkits are now disabled on this server.");

	pSelf->Block().UpdateWeaponkitsVoteOption();
}

void CBlock::ConWhoisId(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	...
	if(ClientId >= 0 && pSelf->Block().m_aWhoisCooldown[ClientId] && Now < pSelf->Block().m_aWhoisCooldown[ClientId])
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "whois", "Please wait before using whois again");
		return;
	}
	if(!pSelf->Block().m_pWhoIs)
	{
	...
	BlockClientAddr(pSelf->Server(), Target, aIp, sizeof(aIp));
	if(aIp[0] == '\0')
	{
	...
	pSelf->Block().m_vWhoisResults.push_back(pRes);
	pSelf->Block().m_pWhoIs->CmdWhoisStr(ClientId, /*Mode=*/0, Cutoff, aIp, pRes);
	if(ClientId >= 0)
	{
		int Sec = std::clamp(g_Config.m_SvWhoisCooldownSec, 0, 300);
		pSelf->Block().m_aWhoisCooldown[ClientId] = Sec > 0 ? Now + Sec * pSelf->Server()->TickSpeed() : 0;
	}
}

void CBlock::ConWhoisIp(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientId = pResult->m_ClientId;
	if(ClientId < 0)
	...
	str_copy(pRes->m_aTag, "whois", sizeof(pRes->m_aTag));
	pSelf->Block().m_vWhoisResults.push_back(pRes);
	pSelf->Block().m_pWhoIs->CmdWhoisStr(ClientId, /*Mode=*/0, Cutoff, pIp, pRes);
	if(ClientId >= 0)
	{
		int Sec = std::clamp(g_Config.m_SvWhoisCooldownSec, 0, 300);
		pSelf->Block().m_aWhoisCooldown[ClientId] = Sec > 0 ? Now + Sec * pSelf->Server()->TickSpeed() : 0;
...
void CBlock::ConWhoisName(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientId = pResult->m_ClientId;
	if(ClientId < 0)
	...
	pSelf->Block().m_vWhoisResults.push_back(pRes);
	// Mode=1 for name
	pSelf->Block().m_pWhoIs->CmdWhoisStr(ClientId, /*Mode=*/1, /*Cutoff=*/0, pName, pRes);
	if(ClientId >= 0)
	{
		int Sec = std::clamp(g_Config.m_SvWhoisCooldownSec, 0, 300);
		pSelf->Block().m_aWhoisCooldown[ClientId] = Sec > 0 ? Now + Sec * pSelf->Server()->TickSpeed() : 0;
...
void CBlock::ConWhoisPurge(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int ClientId = pResult->m_ClientId;
	...
	pSelf->Block().m_pWhoIs->PurgeNow(Months);
}

void CBlock::CreateExplosionVisual(vec2 Pos, CClientMask Mask) const
{
...
void CBlock::CreateStripline(char *pDst, int DstSize, const char *pTitle)
{
...
CPlayer *CBlock::GetPlayer(int ClientID) const
{
...
CPlayer *CBlock::GetPlayerByName(const char *pName) const
{
...
bool CBlock::HandleCosmeticsVote(const CNetMsg_Cl_CallVote *pMsg, int ClientId) const
{
...
SHA256_DIGEST CBlock::HashPassword(const char *pPassword)
{
...
void CBlock::HoldJoinMessage(int ClientId) const
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || !GameServer()->m_apPlayers[ClientId])
		return;

	GameServer()->m_apPlayers[ClientId]->Block().m_EntryChecksPending = true;
}

void CBlock::PreShutdownFlush()
{
	...
	if(m_pAccounts)
	{
		...
		for(int i = 0; i < MAX_CLIENTS; ++i)
		{
			...
			if(!pPl || !pPl->Block().IsLoggedIn())
				continue;
			CAccountData &Acc = pPl->Block().m_Account;
			...
			char aIp[48] = {0};
			BlockClientAddr(Server(), i, aIp, sizeof(aIp));
...
bool CBlock::DeferVote(const char *pDescription, const char *pCommand)
{
...
void CBlock::ProcessComponentsQueue()
{
...
void CBlock::RegisterBlockChatCommands() const
{
	...
	Console()->Register("contributors", "", CFGFLAG_CHAT, ConContributors, GameServer(), "Show thanks to Block contributors.");
	// upstream dropped /credits entirely; Block keeps its own
...
void CBlock::ReleaseJoinMessage(int ClientId) const
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || !GameServer()->m_apPlayers[ClientId])
		return;

	GameServer()->m_apPlayers[ClientId]->Block().m_EntryChecksPending = false;
	SendPendingJoinMessage(ClientId);
}

void CBlock::RemoveVoteByDescription(const char *pDescription) const
{
...
void CBlock::SendChatAccount(int AccountId, const char *pText, int VersionFlags) const
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(pPlayer->Block().GetAccId() != AccountId)
			continue;
...
void CBlock::SendChatClan(int ClanId, const char *pText, int VersionFlags, int From) const
{
	CNetMsg_Sv_Chat Msg;
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(pPlayer->Block().GetClanId() != ClanId)
			continue;
...
void CBlock::SendCosmeticsVoteOptions(int ClientID) const
{
	if(!g_Config.m_SvVotemenuEnabled)
		return;
...
void CBlock::SendPendingJoinMessage(int ClientId) const
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || !GameServer()->m_apPlayers[ClientId])
		return;

	CPlayer *pPlayer = GameServer()->m_apPlayers[ClientId];
	if(!pPlayer->Block().m_JoinMsgPending || pPlayer->Block().m_EntryChecksPending)
		return;

	pPlayer->Block().m_JoinMsgPending = false;
	// 0.7 clients only get the message from us if they did not print it themselves
	if(auto *pBwController = dynamic_cast<CGameControllerBlock *>(GameServer()->m_pController))
	{
		pBwController->SendJoinMessage(pPlayer,
			CGameContext::FLAG_SIX | (pPlayer->Block().m_JoinMsgSilentForSixup ? CGameContext::FLAG_SIXUP : 0));
	}
}

void CBlock::SetVoteDescriptionAtIndex(int *pIndex, const char *pStr, CNetMsg_Sv_VoteOptionListAdd *pOptionMsg)
{
	// forward to the existing free function to keep logic in one place
	::SetVoteDescriptionAtIndex(pIndex, pStr, pOptionMsg);
}

void CBlock::UpdateLMBVoteOption() const
{
...
void CBlock::UpdateWeaponkitsVoteOption() const
{
...
bool CBlock::isInEvent(int pPlayerID)
{
...
int CBlock::GetTilePositions(int TileID, CGameContext *pSelf, std::vector<vec2> &Result)
...
int CBlock::GetSwitchTilePositions(int Type, int Delay, int Number, CGameContext *pSelf, std::vector<vec2> &Result)
{ // use a vector reference as a parameter
	// std::vector<vec2> result; // no need to declare a local vector
	if(Type < 0 || Type > 255)
	{
...
void CBlock::OnTickEarly()
{
	for(const auto &Component : g_ComponentRegistry.Active())
		Component->OnTick();
}

void CBlock::OnTickAfterController()
{
	BLOCK_OnTick();
	m_Animations.Tick();
	m_ZoneManager.Tick();
...
bool CBlock::SkipVoteParticipant(int ClientId) const
{
...
void CBlock::OnPlayerTick(int ClientId) const
{
	...
	if(!pPlayer)
		return;
	CBlockPlayer &Block = pPlayer->Block();

	if(Block.IsLoggedIn())
	{
		Block.SetPlayerPlaytime(Block.GetPlayerPlaytime() + 1);

		if(Block.ExpireVipIfDue(time_timestamp()))
			SendChatTarget(ClientId, "Your VIP has expired. You can buy another week in the shop.");

		int PassiveTime = Block.GetPlayerPassive();
		if(PassiveTime > 0)
		{
			int NewPassive = PassiveTime - 1;
			Block.SetPlayerPassive(NewPassive);
			if(NewPassive == 0)
				SendChatTarget(ClientId, "Your account passive protection has expired.");
		}
	}
	if(Block.m_LocalPassiveDuration > 0)
	{
		Block.m_LocalPassiveDuration--;
		if(Block.m_LocalPassiveDuration == 0)
			SendChatTarget(ClientId, "Your passive protection has expired.");
	}
	if(Block.m_PassiveRaceCooldown > 0)
	{
		Block.m_PassiveRaceCooldown--;
		if(Block.m_PassiveRaceCooldown == 0 && Block.m_PassivePendingGrant)
		{
			Block.m_PassivePendingGrant = false;
			if(!Block.IsLoggedIn())
				Block.m_LocalPassiveDuration = 7200;
			else
				Block.SetPlayerPassive(7200);
			SendChatTarget(ClientId, "Wayblock Protection is now active for 2 hours!");
		}
	}
	if(Block.m_PassiveRemoverUseCooldown > 0)
		Block.m_PassiveRemoverUseCooldown--;
	if(Block.m_RandomCosmeticDuration > 0)
	{
		Block.m_RandomCosmeticDuration--;
		if(Block.m_RandomCosmeticDuration == 0)
		{
			Block.SetSkinMani(-1);
			Block.SetKnockout(-1);
			Block.SetGunDesign(-1);
			Block.m_RandomCosmeticSkinmani = -1;
			Block.m_RandomCosmeticKnockout = -1;
			Block.m_RandomCosmeticGundesign = -1;
...
bool CBlock::OnTeamChat(int ClientId, const char *pMessage) const
{
	CPlayer *pPlayer = GetPlayer(ClientId);
	if(!pPlayer)
		return true;

	if(!pPlayer->Block().IsLoggedIn() || pPlayer->Block().GetClanId() <= 0)
	{
	...
	SendChatClan(pPlayer->Block().GetClanId(), aCensored, 3 /* FLAG_SIX | FLAG_SIXUP */, ClientId);
	return true;
}

bool CBlock::OnPublicChat(int ClientId, const char *pMessage) const
{
...
bool CBlock::OwnsVoteUi(int ClientId) const
{
...
bool CBlock::VoteOnCooldown(int ClientId, const char *pCmd)
{
...
void CBlock::FormatDuration(int Seconds, char *pBuf, size_t Size)
{
...
bool CBlock::IsChatBlocked(int ClientId) const
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || !GameServer()->m_apPlayers[ClientId])
		return false;
	if(Server()->GetAuthedState(ClientId) > 0)
...
bool CBlock::OnJoinSpectators(int ClientId) const
{
...
bool CBlock::BlocksSelfKill(int ClientId)
{
	if(auto pManager = g_ComponentRegistry.Get<COneOnOneManager>())
	{
		if(auto pMatch = pManager->GetMatchForPlayer(ClientId))
...
bool CBlock::OnWhisper(int ClientId, int VictimId, const char *pMessage) const
{
...
void CBlock::OnPublicChatSent(int ClientId, const char *pCensoredMessage, const char *pMessage) const
{
...
bool CBlock::OverrideSpawnPos(int ClientId, vec2 *pSpawnPos)
{
...
void CBlock::OnPlayerEnterMenu(int ClientId) const
{
	if(auto pEvents = g_ComponentRegistry.Get<CEvents>())
	{
		if(auto pActive = pEvents->GetActiveEvent())
...
bool CBlock::OnCharacterDie(CCharacter *pChr, int Killer)
{
...
const char *CBlock::ServerInfoClan(int ClientId)
{
	static char s_aClanName[BLOCK_CLAN_NAME_BUFFER_SIZE];
	s_aClanName[0] = '\0';

	CPlayer *pPlayer = GetPlayer(ClientId);
	if(!pPlayer || !pPlayer->Block().IsLoggedIn() || pPlayer->Block().GetClanId() <= 0 || !Clans())
		return s_aClanName;

	CClansData Clan{};
	if(Clans()->GetClanSnapshotById(pPlayer->Block().GetClanId(), Clan))
		str_copy(s_aClanName, Clan.m_ClanName, sizeof(s_aClanName));
	return s_aClanName;
}

int CBlock::ServerInfoScore(int ClientId) const
{
	CPlayer *pPlayer = GetPlayer(ClientId);
	return (pPlayer && pPlayer->Block().IsLoggedIn()) ? pPlayer->Block().GetPlayerLevel() : 0;
}

void CBlock::LogModeration(int ExecutorId, const char *pFmt, ...) const
{
...
void CBlock::OnRaceFinish(CPlayer *pPlayer) const
{
	CBlockPlayer &Block = pPlayer->Block();
	if(!Block.IsLoggedIn() || g_Config.m_SvRaceFinishExp <= 0)
		return;

	const int Now = Server()->Tick();
	const int CooldownTicks = g_Config.m_SvRaceFinishExpCooldown * Server()->TickSpeed();
	const bool OnCooldown = CooldownTicks > 0 && Block.m_LastRaceFinishExpTick > 0 &&
				Now - Block.m_LastRaceFinishExpTick < CooldownTicks;
	const bool SessionCapped = g_Config.m_SvRaceFinishExpMaxPerSession > 0 &&
				   Block.m_RaceFinishExpCount >= g_Config.m_SvRaceFinishExpMaxPerSession;

	if(OnCooldown)
	{
		const int SecondsLeft = (CooldownTicks - (Now - Block.m_LastRaceFinishExpTick)) / Server()->TickSpeed() + 1;
	...
	Block.AddPlayerExp(Amount);
	Block.m_LastRaceFinishExpTick = Now;
	Block.m_RaceFinishExpCount++;
	SendChatTarget(pPlayer->GetCid(), "+%d EXP for finishing the race!", Amount);
}

void CBlock::OnCharacterDied(CCharacter *pChr, int Killer, int Weapon)
{
...
void CBlock::OnCharacterTakeDamage(CCharacter *pChar, vec2 Force, int Dmg, int From, int Weapon)
{
...
bool CBlock::ExplosionSkipsTarget(int Owner, CCharacter *pTarget) const
{
...
bool CBlock::OnSnapProjectile(int Type, int Owner, vec2 Pos, vec2 Direction, int EntityId, int SnappingClient)
{
	if(Type != WEAPON_GUN)
		return false;
...
bool CBlock::OnProjectileGunImpact(int Owner, vec2 Pos, vec2 Direction, CCharacter *pTargetChr)
{
	// some gundesigns make the tee they hit react
	if(pTargetChr && Owner >= 0 && GameServer()->m_apPlayers[Owner])
	{
		if(GameServer()->m_apPlayers[Owner]->Block().GetGunDesign() == CCosmeticsHandler::GUNDESIGN_HEART)
			GameServer()->SendEmoticon(pTargetChr->GetPlayer()->GetCid(), EMOTICON_HEARTS, -1);
	}
	return Cosmetics()->DoGundesign(Owner, Pos, Direction);
}

CCharacter *CBlock::FilterHitTarget(CCharacter *pOwnerChar, CCharacter *pTargetChr) const
{
	if(pOwnerChar && pOwnerChar->Core()->BlockNoContact())
		return nullptr;
	if(pTargetChr && pTargetChr->Core()->BlockNoContact())
		return nullptr;
	return pTargetChr;
}

CCharacter *CBlock::IntersectLaserTarget(vec2 Pos0, vec2 Pos1, vec2 &NewPos, CCharacter *pOwnerChar, int CollideWith, bool DontHitSelf) const
{
	const bool SelfPassive = pOwnerChar && pOwnerChar->Core()->BlockNoContact();
...
void CBlock::OnLaserHit(int Owner, CCharacter *pHit)
{
	if(pHit)
		BlockTracker().OnPlayerImpacted(pHit->GetOwnerId(), Owner);
}

void CBlock::SnapLaserObject(const CSnapContext &Context, int SnapId, const vec2 &To, const vec2 &From,
...
void CBlock::OnCharacterSpawn(CCharacter *pChr) const
{
	...
	pChr->Block().m_CurrentKillingSpree = 0;

	for(const auto &Component : g_ComponentRegistry.Active())
		Component->OnCharacterSpawn(pPlayer->GetCid(), pChr->m_Pos);
}

int CBlock::SnapPlayerScore(int SnappingClient, CPlayer *pPlayer) const
{
	CBlockPlayer &Block = pPlayer->Block();
	...
	if(Block.m_Score.has_value())
	{
		// Shift the time by a second if the player actually took 9999 seconds.
		if(Block.m_Score.value() == 9999)
			Score = -10000;
		else
			Score = -Block.m_Score.value();
	}
	else
	{
		Score = -9999;
	...
	if(const auto &Mgr = g_SnapCache.m_OneOnOne; Mgr)
	{
	...
	{
		if(const auto &Events = g_SnapCache.m_Events; Events)
		{
	...
	if(!ScoreSetFromEvent)
	{
		if(Block.IsLoggedIn())
		{
			...
			int OwnScore;
			switch(Block.m_ScoreDisplayMode)
			{
			case 1: OwnScore = Block.GetPlayerBlockpoints(); break;
			default: OwnScore = Block.GetPlayerLevel(); break;
			}
			Block.m_Score = OwnScore;
			...
			int ViewerMode = Block.m_ScoreDisplayMode; // fallback for self-snap / demo
			...
			if(SnappingClient >= 0 && SnappingClient < MAX_CLIENTS && SnappingClient != ClientId)
			{
				if(CPlayer *pSnapper = GameServer()->m_apPlayers[SnappingClient])
					ViewerMode = pSnapper->Block().m_ScoreDisplayMode;
			}
			switch(ViewerMode)
			{
			case 1: Score = Block.GetPlayerBlockpoints(); break;
			default: Score = Block.GetPlayerLevel(); break;
			}
		}
		else
		{
			Block.m_Score = Score = 0;
			Server()->SetClientScore(ClientId, Score);
		}
	}
...
void CBlock::OnSnapGameInfo(int SnappingClient, CNetObj_GameInfo *pGameInfo)
{
	if(SnappingClient < 0)
		return;
	const auto &EventsAccessor = g_SnapCache.m_Events;
	if(!EventsAccessor)
		return;
...
void CBlock::OnSnapGameInfoEx(int SnappingClient, CNetObj_GameInfoEx *pGameInfoEx)
{
	// Block is its own gametype, not DDRace/DDNet, and has no race timer. The
	// gametype flags only pick the client's *defaults*; every behaviour we care
...
void CBlock::AddIpMuteSilent(const NETADDR *pAddr, int Secs, const char *pReason) const
{
	if(Secs <= 0)
		return;
...
int CBlock::GetRemainingMuteSecondsPublic(int ClientId) const
{
	if(ClientId < 0 || ClientId >= MAX_CLIENTS || !GameServer()->m_apPlayers[ClientId])
		return 0;
...
void CBlock::SendBroadcast(int To, const char *pText) const
{
	GameServer()->Block().SendBroadcast(pText, To);
}

void CBlock::SendChatCmdGroupStart(int ClientId) const
{
	CNetMsg_Sv_CommandInfoGroupStart Msg;
	Server()->SendPackMsg(&Msg, MSGFLAG_VITAL | MSGFLAG_NORECORD, ClientId);
}

void CBlock::SendChatCmdGroupEnd(int ClientId) const
{
	CNetMsg_Sv_CommandInfoGroupEnd Msg;
	Server()->SendPackMsg(&Msg, MSGFLAG_VITAL | MSGFLAG_NORECORD, ClientId);
}

void CBlock::SendChatCmdAdd(const IConsole::ICommandInfo *pCommandInfo, int ClientId) const
{
	if(!pCommandInfo)
		return;
...
void CBlock::SendChatCmdRem(const IConsole::ICommandInfo *pCommandInfo, int ClientId) const
{
...
bool CBlock::AllowServerVoteStreaming(int ClientId) const
{
	(void)ClientId;
	return false;
}

void CBlock::SendVoteListHeader(int ClientId)
{
...
void CBlock::BotJoin(int BotId, const char *pName) const
{
...
/// Fills the server with bots, for load testing.
///
/// Bots occupy a real client slot and go through the same connect, enter, tick
/// and snapshot path as a player, which is what makes them useful for measuring
...
void CBlock::ConBots(IConsole::IResult *pResult, void *pUserData)
{
	...
	// bots take the high slots, so a real player joining still lands low
	for(int i = pSelf->Server()->MaxClients() - 1; i >= 0 && Current < Wanted; i--)
	{
	...
	for(int i = 0; i < pSelf->Server()->MaxClients() && Current > Wanted; i++)
	{
		if(!pSelf->m_apPlayers[i] || !pSelf->m_apPlayers[i]->Block().m_IsNpc)
			continue;
		Block.BotLeave(i, true);
		Current--;
	}

	char aBuf[64];
	str_format(aBuf, sizeof(aBuf), "%d bots on the server", Current);
...
void CBlock::RedirectClient(int ClientId, int Port, bool Force) const
{
	...
	log_info("block-redirect", "forcing client to another port, cid=%d port=%d", ClientId, Port);
...
void CBlock::OnConstruct(bool FirstInit)
{
...
void CBlock::OnConsoleInit()
{
	...
	// Block's rcon commands. These used to be interleaved into upstream's
...
void CBlock::OnInit()
{
	...
	// first, so a database that has never seen this server -- or one from an
	// older build missing a column -- is brought up to date before anything
	// tries to read from it. The pool runs its queue in order.
	m_pAccounts->EnsureSchema();
	m_pClans->EnsureSchema();

	m_pAccounts->ClearLogins();
	m_pClans->LoadAllClans();
}

void CBlock::OnShutdown()
{
	// the cache owns component references; holding them past shutdown would keep
	// components alive after the registry is done with them
	g_SnapCache.Clear();
	...
	// Block persistence. PreShutdownFlush normally does this earlier; this
...
void CBlock::OnClientConnected(int ClientId)
{
	for(const auto &Component : g_ComponentRegistry.Active())
		Component->OnPlayerConnected(ClientId);
}

void CBlock::OnSetAuthed(int ClientId, int Level) const
{
	for(const auto &Component : g_ComponentRegistry.Active())
	{
		if(Level == AUTHED_NO)
	...
	if(Level == AUTHED_NO && GetPlayer(ClientId))
		GetPlayer(ClientId)->Block().ClearCosmetics();
}

void CBlock::OnPostSnap()
{
	for(const auto &Component : g_ComponentRegistry.Active())
		Component->OnPostSnap();
}

void CBlock::OnClientEnter(int ClientId)
{
	for(const auto &Component : g_ComponentRegistry.Active())
...
void CBlock::OnClientDrop(int ClientId, const char *pReason)
{
	// Clear block votemenu
...
void CBlock::OnSnap(int SnappingClient)
{
	for(const auto &Component : g_ComponentRegistry.Active())
...
bool CBlock::IsSilentDrop(int ClientId) const
{
	CPlayer *pPlayer = GameServer()->m_apPlayers[ClientId];
	return pPlayer && pPlayer->Block().m_EntryChecksPending;
}

void CBlock::SendChatTarget(int To, const char *pText) const
{
	GameServer()->SendChatTarget(To, pText);
}

void CBlock::SendChatTeam(int Team, const char *pText) const
{
	GameServer()->SendChatTeam(Team, pText);
}

void CBlock::SendBroadcast(const char *pText, int ClientId, bool IsImportant) const
{
...
bool CBlock::OnCallVote(const CNetMsg_Cl_CallVote *pMsg, int ClientId) const
{
...
bool CBlock::OnVote(const CNetMsg_Cl_Vote *pMsg, int ClientId)
{
...
void CBlock::OnSnapClientInfo(int ClientId, int SnappingClient, CNetObj_ClientInfo *pClientInfo)
{
	CPlayer *pPlayer = GameServer()->m_apPlayers[ClientId];
	if(!pPlayer)
		return;
	...
	// clan tag comes from the Block clan, not from the client's own
	if(Block.IsLoggedIn() && Block.GetClanId() > 0)
	{
		const char *pClanName = "";
		CClansData Clan;
		if(Clans() && Clans()->GetClanSnapshotById(Block.GetClanId(), Clan))
	...
	if(Block.m_IsNpc && SnappingClient != SERVER_DEMO_CLIENT && SnappingClient >= 0 && SnappingClient < MAX_CLIENTS)
	{
		CPlayer *pViewer = GameServer()->m_apPlayers[SnappingClient];
		if(pViewer && !pViewer->Block().m_IsNpc)
		{
	...
	if(pChar && pChar->Block().IsHookRainbowActive())
	{
		const int64_t TickDef = Server()->Tick() - pPlayer->m_DieTick;
		float Freq = 255.0f;
		const float Divider = pChar->Block().GetHookRainbowDivider();
		if(Divider > 0.0f)
			Freq *= Divider;
	...
	if(Block.GetSkinMani() != -1)
	{
		...
		{
			CPlayer *pViewer = GameServer()->m_apPlayers[SnappingClient];
			if(pViewer && pViewer->Block().m_HideCosmetics)
				HideSkinMani = true;
		}
		if(!HideSkinMani)
...
void CBlock::OnSnapDDNetPlayer(int ClientId, CNetObj_DDNetPlayer *pDDNetPlayer) const
{
	...
	CPlayer *pPlayer = GameServer()->m_apPlayers[ClientId];
	if(pPlayer && pPlayer->Block().m_IsNpc)
