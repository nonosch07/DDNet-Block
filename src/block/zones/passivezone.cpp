...
void CPassiveZone::Tick()
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(InZone && !WasInZone && pChar->Core()->m_Passive)
		{
			...
			if(pChar->Block().m_HookedBy >= 0 && pChar->Block().m_HookedBy < MAX_CLIENTS)
			{
				if(CCharacter *pHooker = GameServer()->GetPlayerChar(pChar->Block().m_HookedBy))
				{
		...
		if(m_aProtectionTicks[i] > 0)
		{
			...
			if(m_aProtectionTicks[i] == 0)
			{
				pChar->Block().Core().m_Passive = false;
			}
		}
...
void CPassiveZone::OnCharacterDeath(CCharacter *pCharacter)
{
	...
	pCharacter->Block().Core().m_Passive = false;
	if(pCharacter->GetPlayer())
	{
		pCharacter->GetPlayer()->Block().PromotePassiveIfPending();
	}
}
...
void CPassiveZone::HandleProtection(int ClientID, CPlayer *pPlayer, CCharacter *pChar, bool InZone, bool WasInZone)
{
	...
	bool HasLocal = (pPlayer->Block().m_LocalPassiveDuration > 0);
	bool HasAccount = (pPlayer->Block().IsLoggedIn() && pPlayer->Block().GetPlayerPassive() > 0);
	bool Eligible = (HasLocal || HasAccount) && pPlayer->Block().IsUsingPassiveProtection();
	...
	if(InZone)
	{
		...
		bool PassiveAllowed = (pPlayer->Block().IsLoggedIn() && pPlayer->Block().GetPlayerPassive() > 0 && pPlayer->Block().IsUsingPassiveProtection()) || (!pPlayer->Block().IsLoggedIn() && pPlayer->Block().m_LocalPassiveDuration > 0 && pPlayer->Block().IsUsingPassiveProtection());
		...
		if(!m_ProtectionUsed[ClientID] && s_PassiveEnabledOnEntry[ClientID])
		{
			...
			pChar->Block().Core().m_Passive = true;
		}
		else
		{
			// fix:
