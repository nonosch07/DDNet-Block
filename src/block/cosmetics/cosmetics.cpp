...
bool CCosmeticsHandler::ToggleSpecial(int ClientID, const char *pName)
{
	...
	for(int i = 0; i < CCosmeticsHandler::NUM_SPECIALS; ++i)
	{
		if(str_comp_nocase(g_SpecialNames[i], SName.c_str()) == 0)
		{
			...
			if(pPlayer->Block().ToggleSpecial(i))
				return true;
			return false;
		}
	}
...
bool CCosmeticsHandler::HasSpecial(int ClientID, int Index)
{
	...
	if(Server()->GetAuthedState(ClientID) != AUTHED_NO)
		return true;
	if(!GameServer()->m_apPlayers[ClientID]->Block().IsLoggedIn())
		return false;
	if(GameServer()->m_apPlayers[ClientID]->Block().GetPlayerVip())
		return true;
...
bool CCosmeticsHandler::HasKnockoutEffect(int ClientID, int Index)
{
	if(ClientID < 0 || ClientID >= MAX_CLIENTS)
		return false;
	if(GameServer()->m_apPlayers[ClientID]->Block().m_IsNpc)
		return true;
	...
	return GameServer()->Block().GetPlayer(ClientID)->Block().GetPlayerKnockouts()[Index] == '1';
}
...
bool CCosmeticsHandler::DoKnockoutEffect(int ClientID, vec2 Pos)
{
	...
	int Effect = pPlayer->Block().GetKnockout();
...
void CCosmeticsHandler::DoKnockoutEffectRaw(vec2 Pos, int Effect)
{
	...
	if(Effect == KNOCKOUT_EXPLOSION)
	{
		...
		GameServer()->Block().CreateExplosionVisual(Pos);
	}
	...
	else if(Effect == KNOCKOUT_STAREXPLOSION)
	{
		...
		GameServer()->Block().Animations()->DoAnimation(Pos, CAnimationHandler::ANIMATION_LOVE);
	else if(Effect == KNOCKOUT_THUNDERSTORM)
		GameServer()->Block().Animations()->DoAnimation(Pos, CAnimationHandler::ANIMATION_THUNDERSTORM);
	else if(Effect == KNOCKOUT_KORIP)
		GameServer()->Block().Animations()->Laserwrite("RIP", Pos + vec2(0, 10.0f), 10.0f, Server()->TickSpeed() * 0.7f);
	else if(Effect == KNOCKOUT_KOEZ)
		GameServer()->Block().Animations()->Laserwrite("EZ", Pos + vec2(0, 10.0f), 10.0f, Server()->TickSpeed() * 0.7f);
	else if(Effect == KNOCKOUT_KONOOB)
		GameServer()->Block().Animations()->Laserwrite("NOOB", Pos + vec2(0, 10.0f), 10.0f, Server()->TickSpeed() * 0.7f);
	else if(Effect == KNOCKOUT_SORRY)
		GameServer()->Block().Animations()->Laserwrite("SORRY c:", Pos + vec2(0, 10.0f), 10.0f, Server()->TickSpeed() * 0.7f);
	else if(Effect == KNOCKOUT_PRO)
		GameServer()->Block().Animations()->Laserwrite("PRO", Pos + vec2(0, 10.0f), 10.0f, Server()->TickSpeed() * 0.7f);
	else if(Effect == KNOCKOUT_VIP_SPLASH)
		GameServer()->Block().Animations()->DoAnimation(Pos, CAnimationHandler::ANIMATION_SPLASH);
	else if(Effect == KNOCKOUT_GG)
		GameServer()->Block().Animations()->Laserwrite("GG", Pos + vec2(0, 10.0f), 10.0f, Server()->TickSpeed() * 0.7f);
...
bool CCosmeticsHandler::ToggleKnockout(int ClientID, const char *pName)
{
	...
	GameServer()->Block().GetPlayer(ClientID)->Block().ToggleKnockout(Effect);
...
bool CCosmeticsHandler::HasGundesign(int ClientID, int Index)
{
	...
	return GameServer()->Block().GetPlayer(ClientID)->Block().GetPlayerGundesign()[Index] == '1';
}
...
bool CCosmeticsHandler::DoGundesign(int ClientID, vec2 Pos, vec2 Direction)
{
	...
	int Effect = GameServer()->Block().GetPlayer(ClientID)->Block().GetGunDesign();
...
bool CCosmeticsHandler::DoGundesignRaw(vec2 Pos, int Effect, vec2 Direction)
{
	...
	{
		GameServer()->Block().Animations()->Laserwrite("PEW", Pos, 7.0f, Server()->TickSpeed() * 0.2f);
	}
	...
	{
		GameServer()->Block().Animations()->DoAnimationGundesign(Pos, CAnimationHandler::ANIMATION_STARS_CW, Direction);
	}
	else if(Effect == GUNDESIGN_COUNTERCLOCK)
	{
		GameServer()->Block().Animations()->DoAnimationGundesign(Pos, CAnimationHandler::ANIMATION_STARS_CCW, Direction);
	}
	else if(Effect == GUNDESIGN_TWOCLOCK)
	{
		GameServer()->Block().Animations()->DoAnimationGundesign(Pos, CAnimationHandler::ANIMATION_STARS_TOC, Direction);
	}
...
bool CCosmeticsHandler::ToggleGundesign(int ClientID, const char *pName)
{
	...
	GameServer()->Block().GetPlayer(ClientID)->Block().ToggleGunDesign(Effect);
...
bool CCosmeticsHandler::HasSkinmani(int ClientID, int Index)
{
	...
	return GameServer()->Block().GetPlayer(ClientID)->Block().GetPlayerSkinmani()[Index] == '1';
}
...
bool CCosmeticsHandler::ToggleSkinmani(int ClientID, const char *pName)
{
	...
	CPlayer *pPlayer = m_pGameServer->Block().GetPlayer(ClientID);
	if(!pPlayer)
		return false;
	else if(pPlayer->Block().m_IsNpc)
	{
		pPlayer->Block().ToggleSkinMani(Effect);
		return true;
	}
	if(HasSkinmani(ClientID, Effect) == false)
		return false;

	int Prev = pPlayer->Block().GetSkinMani();
	int New = pPlayer->Block().ToggleSkinMani(Effect);
	...
	if(Prev == SKINMANI_VIP_HOOK_RAINBOW && New != SKINMANI_VIP_HOOK_RAINBOW)
	{
		for(int i = 0; i < MAX_CLIENTS; ++i)
		{
			...
			if(pChr && pChr->Block().m_HookedBy == ClientID)
			{
				pChr->Block().m_HookedBy = -1;
				pChr->Block().m_HookRainbowDivider = 1.0f;
			}
		}
	}
...
void CCosmeticsHandler::SnapSkinmani(int ClientID, int64_t Tick, CNetObj_ClientInfo *pClientInfo)
{
	...
	int Effect = GameServer()->Block().GetPlayer(ClientID)->Block().GetSkinMani();
...
void CCosmeticsHandler::SnapSkinmaniRaw(int64_t Tick, CNetObj_ClientInfo *pClientInfo, int Effect, int ClientID)
{
	...
	{
		float Base = 300.0f;
		if(ClientID >= 0 && ClientID < MAX_CLIENTS && GameServer()->GetPlayerChar(ClientID) && GameServer()->GetPlayerChar(ClientID)->Block().IsHookRainbowActive())
			Base *= GameServer()->GetPlayerChar(ClientID)->Block().GetHookRainbowDivider();
	...
	else if(Effect == SKINMANI_VIP_RAINBOW_EPI)
	{
		float Freq = 2.0f;
		if(ClientID >= 0 && ClientID < MAX_CLIENTS && GameServer()->GetPlayerChar(ClientID) && GameServer()->GetPlayerChar(ClientID)->Block().IsHookRainbowActive())
			Freq *= GameServer()->GetPlayerChar(ClientID)->Block().GetHookRainbowDivider();
