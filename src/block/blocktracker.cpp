...
#include <block/entities/experience.h>
...
bool CBlockTracker::Blocked(int ClientID, int BlockerID)
{
	...
	{
		int VictimClan = pVictimPlayer->Block().GetClanId();
		int BlockerClan = pBlockerPlayer->Block().GetClanId();
	...
	if(BlockIsClientsSameAddr(m_pGameContext->Server(), ClientID, BlockerID) && !g_Config.m_SvAllowExpFromSameIp)
	{
	...
	int64_t AliveTime = CurrentTick - pChr->Block().m_AliveSince;
	...
	m_pGameContext->Block().Cosmetics()->DoKnockoutEffect(m_aTrackedPlayers[ClientID].m_ImpactedClientID, pChr->m_Pos);
...
float CBlockTracker::LevelDiffScale(int KillerID, int VictimID) const
{
	...
	if(!pKiller || !pVictim)
		return 1.0f;
	if(!pKiller->Block().IsLoggedIn() || !pVictim->Block().IsLoggedIn())
		return 1.0f;
	int KLevel = pKiller->Block().GetPlayerLevel();
	int vLevel = pVictim->Block().GetPlayerLevel();
...
void CBlockTracker::KillStreaks(int ClientID, int BlockerID)
{
	...
	pBlockerChr->Block().m_KillStreak++;

	// update hourly best streak (EXP-eligible kills only, i.e. real blocks)
	if(pBlockerChr->Block().m_KillStreak > m_aHourlyStats[BlockerID].m_BestStreak)
		m_aHourlyStats[BlockerID].m_BestStreak = pBlockerChr->Block().m_KillStreak;

	if(CPlayer *pBlockerPlayer = pBlockerChr->GetPlayer())
	{
		if(pBlockerPlayer->Block().IsLoggedIn())
		{
			// only update stored account killstreak if we reached a new maximum
			if(pBlockerChr->Block().m_KillStreak > pBlockerPlayer->Block().GetPlayerKillstreak())
			{
				pBlockerPlayer->Block().SetPlayerKillstreak(pBlockerChr->Block().m_KillStreak);
			}
		}
	}
	if(pBlockerChr->Block().m_KillStreak % g_Config.m_SvKillStreakCount == 0)
	{
		char aBuf[64];
		str_format(aBuf, sizeof(aBuf), "'%s' has a killstreak of %i!", m_pGameContext->Server()->ClientName(BlockerID), pBlockerChr->Block().m_KillStreak);
		m_pGameContext->SendChat(-1, -2, aBuf);
	}

	if(pClientChr->Block().m_KillStreak >= g_Config.m_SvKillStreakCount)
	{
		char aBuf[64];
		str_format(aBuf, sizeof(aBuf), "'%s's killing spree of %d ended by '%s'!", m_pGameContext->Server()->ClientName(ClientID), pClientChr->Block().m_KillStreak, m_pGameContext->Server()->ClientName(BlockerID));
		m_pGameContext->SendChat(-1, -2, aBuf);
	}

	pClientChr->Block().m_KillStreak = 0;
}
...
bool CBlockTracker::OnPlayerKill(int ClientID)
{
	...
	CCharacter *pChr = m_pGameContext->GetPlayerChar(ClientID);
	if(CPlayer *pPlayer = pChr->GetPlayer(); pPlayer && pPlayer->Block().IsLoggedIn())
	{
		...
		if(!InEvent)
			pPlayer->Block().SetPlayerDeaths(pPlayer->Block().GetPlayerDeaths() + 1);
	}
