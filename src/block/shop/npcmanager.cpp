...
int CNpcManager::EnsureNpcAndApplySkinmani(int Index, const vec2 &PreviewPos, const char *pSkinName)
{
	...
	if(N.m_ClientID == -1)
	{
		int DummyID = m_pGameServer->Block().GetNextClientID();
		if(DummyID == -1)
			return -1;
		...
		m_pGameServer->Block().BotJoin(DummyID, "");

		CPlayer *pFakePlayer = m_pGameServer->m_apPlayers[DummyID];
		if(!pFakePlayer)
			return -1;

		pFakePlayer->Block().m_IsNpc = true;
		pFakePlayer->SetAfk(true);

		N.m_ConnectionTick = m_pGameServer->Server()->Tick();
	}

	CPlayer *pPlayer = m_pGameServer->Block().GetPlayer(N.m_ClientID);
	if(!pPlayer)
		return -1;
	...
	if(!N.m_Toggled)
	{
		m_pGameServer->Block().Cosmetics()->ToggleSkinmani(N.m_ClientID, pSkinName);
		N.m_Toggled = true;
	}
	...
	if(pChr && !N.m_Teleported)
	{
		...
		{
			m_pGameServer->Block().Teleport(pChr, PreviewPos);
			N.m_Teleported = true;
		}
	}
...
void CNpcManager::Tick()
{
	...
	for(auto &N : m_vNpcs)
	{
		...
		{
			m_pGameServer->Block().Teleport(pChr, N.m_PreviewPos);
		}

		pChr->Block().Core().m_Vel = vec2(0, 0);
...
void CNpcManager::RemoveAll()
{
	...
	for(auto &N : m_vNpcs)
	{
		...
		if(ClientID >= 0 && ClientID < (int)MAX_CLIENTS)
		{
			...
			if(p && p->Block().m_IsNpc)
			{
				m_pGameServer->Block().BotLeave(ClientID);
			}
		}
