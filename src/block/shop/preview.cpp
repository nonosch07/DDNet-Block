...
void CShopPreview::DisplayPickupGundesigns()
{
	...
	for(int i = 0; i < NumGundesigns; i++)
	{
		int Price = 0, Level = 0;
		vec2 PreviewPos;
		if(!m_pGameContext->Block().Cosmetics()->ShopInfoGundesign(i, Price, Level, PreviewPos))
			continue;
...
void CShopPreview::DisplayFastGundesigns()
{
	for(int i = 0; i < CCosmeticsHandler::NUM_GUNDESIGNS; i++)
	{
		...
		case CCosmeticsHandler::GUNDESIGN_PEW:
			GameServer()->Block().Cosmetics()->DoGundesignRaw(PreviewPos, i, vec2(1, 0));
			break;
		default:
			break;
		}
...
void CShopPreview::DisplaySlowKnockouts()
{
	...
	for(int Ko : SlowKnockouts)
	{
		int Price = 0, Level = 0;
		vec2 PreviewPos;
		if(m_pGameContext->Block().Cosmetics()->ShopInfoKnockout(Ko, Price, Level, PreviewPos))
		{
			GameServer()->Block().Cosmetics()->DoKnockoutEffectRaw(PreviewPos, Ko);
		}
	}
}
...
void CShopPreview::DisplaySkinManipulations()
{
	...
	for(int i = 0; i < Num; i++)
	{
		...
		if(!m_pGameContext->Block().Cosmetics()->ShopInfoSkinmani(i, Price, Level, PreviewPos))
			continue;
...
void CShopPreview::DisplayPriceLevel(const vec2 &PreviewPos, int Price, int Level)
{
	...
	GameServer()->Block().Animations()->RemoveAnimationsNear(TextPos, 200.0f);

	const int Lifetime = GameServer()->Server()->TickSpeed() * 3;
	GameServer()->Block().Animations()->Laserwrite(aBuf, TextPos, 5.0f, Lifetime, true);
}
...
void CShopPreview::DisplayPricesAndLevels()
{
	...
	for(int i = 0; i < CCosmeticsHandler::NUM_KNOCKOUTS; i++)
	{
		int Price = 0, Level = 0;
		vec2 PreviewPos;
		if(m_pGameContext->Block().Cosmetics()->ShopInfoKnockout(i, Price, Level, PreviewPos))
			DisplayPriceLevel(PreviewPos, Price, Level);
	}
