...
bool CClanManager::EnsureSchemaThread(IDbConnection *pSqlServer, const ISqlData *pGameData, Write w, char *pError, int ErrorSize)
{
	// see CAccounts::EnsureSchemaThread: the backup connection is SQLite and has
	// no business holding clans
	...
	// clans are referenced from Block_accounts_progress.clanID, but without a
	// foreign key: a clan can be disbanded while its members are offline, and
	// their stored id is cleaned up on the next load instead.
	const char *pCreate =
		...
		"  PRIMARY KEY (`id`),"
		"  UNIQUE KEY `ux_clans_name` (`name`)"
		")" BLOCK_ENGINE_COLLATE;

	if(!RunDdl(pSqlServer, pCreate, pError, ErrorSize))
	{
		dbg_msg("sql", "could not create the clan table, so clans will not work: %s", pError);
		return false;
	}
	dbg_msg("sql", "clan schema is up to date");
	return true;
}

void CClanManager::EnsureSchema()
{
	...
	m_pPool->ExecuteWrite(EnsureSchemaThread, std::make_unique<CSchemaRequest>(), "create clan schema");
}
...
std::shared_ptr<CClanResult> CClanManager::NewSqlClanResult(int ClientId)
{
	...
	pCurPlayer->Block().m_ClanQueryResult.push(pResult);
	return pResult;
}
...
void CClanManager::ClanLeave(int ClientId)
{
	...
	if(RateLimitPlayer(ClientId))
		return;
	ExecClanThread(ClanLeaveThread, "clan leave", ClientId, "", pPlayer->Block().GetPlayerName(), pPlayer->Block().GetAccId(), pPlayer->Block().m_Account.m_ClanId);
}
...
void CClanManager::SetAuthLevel(int ClientId, const char *AccountName, int NewAuthLevel, int ClanId)
{
	...
	if(!pPlayer)
		return;
	ExecClanThread(SetAuthLevelThread, "auth level thread", ClientId, "", AccountName, pPlayer->Block().GetAccId(), ClanId, NewAuthLevel);
}
...
void CClanManager::TransferLeadership(int ClientId, const char *AccountName, int ClanId)
{
	...
	if(!pPlayer)
		return;
	ExecClanThread(TransferLeadershipThread, "transfer leadership", ClientId, "", AccountName, pPlayer->Block().GetAccId(), ClanId);
}
...
void CClanManager::RenameClan(int ClientId, int ClanId, const char *pNewClanName)
{
	...
	{
		pRequest->m_AccountId = pPlayer->Block().GetAccId();
	}
...
void CClanManager::RemoveFromClan(int ClientId, const char *AccountName, int ClanId)
{
	...
	if(!pPlayer)
		return;
	ExecClanThread(RemoveFromClanThread, "remove from clan", ClientId, "", AccountName, pPlayer->Block().GetAccId(), ClanId);
}
...
bool CClanManager::CreateClanThread(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	char aNormalizedName[BLOCK_CLAN_NAME_BUFFER_SIZE];
	...
	if(NameChars < 3 || NameChars > BLOCK_CLAN_NAME_MAX_LENGTH)
	{
	...
	// push the new clan data into the in-memory container
	{
		...
		char aNameLower[BLOCK_CLAN_NAME_BUFFER_SIZE];
...
bool CClanManager::DeleteClanThread(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	if(pData->m_pClanManager)
	{
		...
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if(pPlayer && pPlayer->Block().m_Account.m_ClanId == pData->m_ClanId)
			{
...
bool CClanManager::AssignClanThread(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	if(pData->m_ClientId >= 0 && pData->m_ClanId > 0)
	{
		...
		int InviterAccId = pInviter->Block().GetAccId();
	...
	if(pData->m_pClanManager)
	{
		...
		pGameServer->Block().SendChatClan(pData->m_ClanId, aBroadcast);
	}
...
bool CClanManager::RemoveFromClanThread(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	for(int i = 0; pGameServer && i < MAX_CLIENTS; i++)
	{
		...
		if(pTarget && pTarget->Block().IsLoggedIn() && str_comp(pTarget->Block().m_Account.m_aName, pData->m_aUsername) == 0)
		{
			KickedClientId = i;
			break;
		}
		...
		if(KickedClientId != -1)
		{
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				...
				if(!pPl || !pPl->Block().IsLoggedIn())
					continue;
				if(pPl->Block().GetClanId() != pData->m_ClanId)
					continue;
...
bool CClanManager::RenameClanThread(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	if(NewNameLen < 3 || NewNameLen > BLOCK_CLAN_NAME_MAX_LENGTH)
	{
		pResult->SetVariant(CClanResult::DIRECT);
		str_copy(pResult->m_aaMessages[0], "Clan name must be between 3 and 11 characters (after trimming).", sizeof(pResult->m_aaMessages[0]));
		return true;
...
bool CClanManager::LoadClansThread(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	char aBuf[1024];
	// Use prefixed Block clans table
	...
	while(!End)
	{
		...
		if(!End)
		{
			...
			if(str_length(aNameBuf) > BLOCK_CLAN_NAME_MAX_LENGTH)
			{
				dbg_msg("clan", "WARNING: clan id=%d name '%s' exceeds %d byte limit (%d bytes), skipping",
					Data.m_Id, aNameBuf, BLOCK_CLAN_NAME_MAX_LENGTH, str_length(aNameBuf));
				continue;
			}
...
bool CClanManager::ShowTopClansThread(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	{
		*pError = '\0';
		char aClanName[BLOCK_CLAN_NAME_BUFFER_SIZE];
		int Level = 0;
...
bool CClanManager::ShowClanMembersThread(IDbConnection *pSqlServer, const ISqlData *pGameData, char *pError, int ErrorSize)
{
	...
	char aClanName[BLOCK_CLAN_NAME_BUFFER_SIZE] = {0};
	if(pData->m_pClanManager)
	{
