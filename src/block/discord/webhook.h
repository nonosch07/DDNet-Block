/* (c) Block contributors. See licence.txt in the root of the distribution for more information. */
#ifndef BLOCK_DISCORD_WEBHOOK_H
#define BLOCK_DISCORD_WEBHOOK_H

#include <base/types.h>

#include <engine/engine.h>
#include <engine/http.h>

#include <memory>

// Job-backed Discord Webhook client
//
// Configuration (in server cfg):
//   sv_discord_enabled 1
// Optional:
//   sv_discord_webhook_username "Block Bot"
//   sv_discord_webhook_avatar   "https://.../avatar.png"
//   sv_discord_tts 0/1
//   sv_discord_thread_id "<thread id>"
//
// Usage:
//   CDiscordWebhook Discord(Engine(), Http());
//   Discord.Send("Hello");
class CDiscordWebhook
{
public:
	struct SSendOptions
	{
		// Overrides for payload; if nullptr or empty, falls back to config values.
		const char *m_pUsername = nullptr;
		const char *m_pAvatarUrl = nullptr;
		// If set to -1, use config; otherwise force.
		int m_Tts = -1; // 0/1
		const char *m_pThreadId = nullptr;
		// Required: explicit webhook URL to send to (no global fallback)
		const char *m_pWebhookUrl = nullptr;
	};

private:
	class CSendJob;

	IEngine *m_pEngine = nullptr;
	IHttp *m_pHttp = nullptr;

public:
	CDiscordWebhook(IEngine *pEngine, IHttp *pHttp);

	bool IsConfigured(const char *pUrlOverride) const;

	// Queue a message send (non-blocking). Returns immediately.
	void Send(const char *pContent, const SSendOptions *pOpt = nullptr);
	void Send(const char *pContent, SSendOptions Opt) { Send(pContent, &Opt); }

	void BroadcastCmd(const char *pCmd, const char *pExec, const char *pArgs);

	// One-line relay to the rcon-log webhook, for engine-side code that has an
	// engine and an http but no CGameContext to reach CBlock through.
	// Does nothing when the webhook is not configured.
	[[gnu::format(printf, 3, 4)]] static void SendRconLog(IEngine *pEngine, IHttp *pHttp, const char *pFmt, ...);
};

#endif // BLOCK_DISCORD_WEBHOOK_H
