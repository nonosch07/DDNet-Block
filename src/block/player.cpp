...
#include <block/cosmetics/specials/ball.h>
#include <block/cosmetics/specials/crown.h>
#include <block/cosmetics/specials/epiccircle.h>
#include <block/cosmetics/specials/flag.h>
#include <block/cosmetics/specials/halo.h>
...
CGameContext *CBlockPlayer::GameServer() const { return m_pPlayer->GameServer(); }
IServer *CBlockPlayer::Server() const { return m_pPlayer->Server(); }
int CBlockPlayer::GetCid() const { return m_pPlayer->GetCid(); }
CCharacter *CBlockPlayer::GetCharacter() const { return m_pPlayer->GetCharacter(); }

int CBlockPlayer::GetClanLevel() const
{
	CClansData Tmp;
	if(m_Account.m_ClanId > 0 && GameServer()->Block().Clans()->GetClanSnapshotById(m_Account.m_ClanId, Tmp))
		return Tmp.m_Level;
	return 0;
}

int CBlockPlayer::GetClanExperience() const
{
	CClansData Tmp;
	if(m_Account.m_ClanId > 0 && GameServer()->Block().Clans()->GetClanSnapshotById(m_Account.m_ClanId, Tmp))
...
void CBlockPlayer::GiveFlag(int DurationMinutes)
{
...
void CBlockPlayer::GiveTemporarySpecial(int SpecialIndex, int DurationMinutes)
{
...
void CBlockPlayer::BlockProcessAccountsResult(CAccountResult &Result)
{
	if(Result.m_Success)
	{
		...
		case CAccountResult::DIRECT:
			for(auto &aMessage : Result.m_aaMessages)
			{
				if(aMessage[0] == 0)
					break;
				GameServer()->Block().SendChatTarget(GetCid(), aMessage);
			}
			break;
		...
		case CAccountResult::TOP_MESSAGES:
			if(m_CaptureTopToMenu)
			{
				...
				GameServer()->Block().ClearVotes(GetCid());
				GameServer()->ProgressVoteOptions(GetCid());
				GameServer()->Block().SendCosmeticsVoteOptions(GetCid());
			}
			else
			{
				for(auto &aMessage : Result.m_aaMessages)
		...
		case CAccountResult::LOGGED_IN_ALREADY:
			...
			{
				GameServer()->Block().SendChatTarget(GetCid(), "Account is already being used on this server.");
				break;
			}
			GameServer()->Block().SendChatTarget(GetCid(), "Account is already being used on other server.");
			break;

		case CAccountResult::LOGIN_WRONG_PASS:
			GameServer()->Block().SendChatTarget(GetCid(), "Wrong username or password.");
			break;
...
void CBlockPlayer::BlockProcessClansResult(CClanResult &Result)
{
	...
	if(Result.m_Success || Result.m_aaMessages[0][0] != '\0')
	{
		...
		case CClanResult::ACTION_UPDATE_PLAYER_BY_CLIENT:
			if(Result.m_ActionClientId >= 0 && Result.m_ActionClientId < MAX_CLIENTS)
			{
				...
				if(pTarget)
				{
					const char *pPlayerName = pTarget->Block().GetPlayerName();

					pTarget->Block().m_Account.m_ClanId = Result.m_ActionNewClanId;
					pTarget->Block().m_Account.m_AuthLevel = static_cast<ClanAuthLevel>(Result.m_ActionNewAuthLevel);
					...
					if(DiscordConfigured && Result.m_Success)
					{
						...
						{
							std::string ClanName = GameServer()->Block().Clans()->GetClanNameCopy(Result.m_ActionNewClanId);
							char aMsg[512];
		...
		case CClanResult::ACTION_UPDATE_PLAYER_BY_NAME:
			if(Result.m_ActionPlayerName[0] != '\0')
			{
				...
				{
					if(pTarget && pTarget->Block().IsLoggedIn() && str_comp(pTarget->Block().m_Account.m_aName, Result.m_ActionPlayerName) == 0)
					{
						int PrevClan = pTarget->Block().GetClanId();
						const char *pPlayerName = pTarget->Block().GetPlayerName();
						pTarget->Block().m_Account.m_ClanId = Result.m_ActionNewClanId;
						pTarget->Block().m_Account.m_AuthLevel = static_cast<ClanAuthLevel>(Result.m_ActionNewAuthLevel);
						...
						if(DiscordConfigured && Result.m_Success)
						{
							...
							{
								std::string ClanName = GameServer()->Block().Clans()->GetClanNameCopy(PrevClan);
								char aMsg[512];
		...
		case CClanResult::ACTION_UPDATE_TWO_PLAYERS:
		{
			...
			if(Result.m_ActionPlayerName[0] != '\0')
			{
				for(auto *pTarget : GameServer()->m_apPlayers)
				{
					if(pTarget && pTarget->Block().IsLoggedIn() && str_comp(pTarget->Block().m_Account.m_aName, Result.m_ActionPlayerName) == 0)
					{
						...
						const char *pPlayerName = pTarget->Block().GetPlayerName();
						pTarget->Block().m_Account.m_ClanId = Result.m_ActionNewClanId;
						pTarget->Block().m_Account.m_AuthLevel = static_cast<ClanAuthLevel>(Result.m_ActionNewAuthLevel2);
			...
			if(Result.m_ActionClientId >= 0 && Result.m_ActionClientId < MAX_CLIENTS)
			{
				...
				if(pIssuer)
				{
					pIssuer->Block().m_Account.m_ClanId = Result.m_ActionNewClanId;
					pIssuer->Block().m_Account.m_AuthLevel = static_cast<ClanAuthLevel>(Result.m_ActionNewAuthLevel);
					...
					if(DiscordConfigured && Result.m_Success)
					{
						...
						{
							char aMsg[512];
							str_format(aMsg, sizeof(aMsg), "[CLAN] Transferred: %s was demoted to co-leader of clan id %d", pIssuer->Block().GetPlayerName(), Result.m_ActionNewClanId);
							CDiscordWebhook::SSendOptions Opt;
							Opt.m_pWebhookUrl = pLogsUrl;
							Discord.Send(aMsg, Opt);
						}
		...
		case CClanResult::ACTION_RESET_CLAN_PLAYERS:
			if(Result.m_ActionResetClanId > 0)
			{
				...
				if(DiscordConfigured && Result.m_Success)
				{
					std::string ClanName = GameServer()->Block().Clans()->GetClanNameCopy(Result.m_ActionResetClanId);
					char aMsg[512];
				...
				for(auto *pTarget : GameServer()->m_apPlayers)
				{
					if(pTarget && pTarget->Block().IsLoggedIn() && pTarget->Block().m_Account.m_ClanId == Result.m_ActionResetClanId)
					{
						pTarget->Block().m_Account.m_ClanId = 0;
						pTarget->Block().m_Account.m_AuthLevel = ClanAuthLevel::NONE;
					}
				}
			}
			break;
		case CClanResult::ACTION_NOTIFY_CLAN_RENAME:
			if(GetClanId() > 0)
			{
				...
				for(int i = 0; i < MAX_CLIENTS; ++i)
				{
					...
					if(pMember && pMember->Block().IsLoggedIn() && pMember->Block().GetClanId() == GetClanId())
					{
						GameServer()->Block().SendChatTarget(i, aBuf);
					}
				}
				if(DiscordConfigured && Result.m_Success)
				{
		...
		if(Result.m_Success && Result.m_ActionChargeClientId >= 0 && Result.m_ActionChargeAmount > 0)
		{
			...
			if(Cid >= 0 && Cid < MAX_CLIENTS)
			{
				...
				if(pCharger && pCharger->Block().IsLoggedIn())
				{
					int Cost = Result.m_ActionChargeAmount;
					if(pCharger->Block().GetPlayerBlockpoints() >= Cost)
					{
						pCharger->Block().SetPlayerBlockpoints(pCharger->Block().GetPlayerBlockpoints() - Cost);
						GameServer()->Block().Accounts()->Save(Cid, &pCharger->Block().m_Account);
						char aBuf[128];
						...
						GameServer()->Block().SendChatTarget(Cid, aBuf);
					}
					else
					{
						if(Result.m_ActionChargeAmount == g_Config.m_SvClanRenamePrice)
							GameServer()->Block().SendChatTarget(Cid, "Rename fee could not be charged due to insufficient blockpoints after rename.");
						else if(Result.m_ActionChargeAmount == g_Config.m_SvClanCreatePrice)
							GameServer()->Block().SendChatTarget(Cid, "Creation fee could not be charged due to insufficient blockpoints after creation.");
						else
							GameServer()->Block().SendChatTarget(Cid, "Fee could not be charged due to insufficient blockpoints.");
					}
				}
			}
		}
			...
			for(int i = 0; i < MAX_CLIENTS; i++)
			{
				...
				if(pMember && pMember->Block().IsLoggedIn() && pMember->Block().GetClanId() == GetClanId())
				{
					for(auto &aMessage : Result.m_aaMessages)
					{
						if(aMessage[0] == 0)
							break;
						GameServer()->Block().SendChatTarget(i, aMessage);
					}
				}
			}
			break;
...
void CBlockPlayer::BlockProcessAdminCommandResult(CAdminCommandResult &Result) const
{
	if(Result.m_Success) // SQL request was successful
	{
		...
		case CAdminCommandResult::BROADCAST:
			...
			// 	GameServer()->Block().SendBroadcast(Result.m_aBroadcast, -1);
			break;
...
void CBlockPlayer::OnPlayerLogin()
{
	// KillCharacter(); - no need to kill since we allow using /login only at spawn
	GameServer()->Block().SendChatTarget(GetCid(), "Login successfully");

	// Log account info to whois now that we have it (join event fires before login)
	if(GameServer()->Block().WhoIs())
		GameServer()->Block().WhoIs()->LogJoin(GetCid(), "login");
	...
	if(m_Account.m_WeeklyExpBoostUntil > 0)
	{
		...
		if(NowPlaytime < m_Account.m_WeeklyExpBoostUntil)
		{
			...
			if(RemainingMinutes > 0)
			{
				...
				GameServer()->Block().SendChatTarget(GetCid(), aBuf);
			}
		}
		else
		{
	...
	if(m_Account.m_Id > 0)
	{
		...
		char aIp[48] = {0};
		BlockClientAddr(Server(), GetCid(), aIp, sizeof(aIp));
		if(aIp[0])
		{
			bool Invalid = false;
			for(int i = 0; aIp[i]; ++i)
...
void CBlockPlayer::OnPlayerSave(bool Logout)
{
	...
	GameServer()->Block().Accounts()->Save(GetCid(), &m_Account);
	if(Logout)
		GameServer()->Block().Accounts()->Logout(GetCid(), m_Account.m_Id);
}

void CBlockPlayer::OnPlayerLogout()
{
	if(!IsLoggedIn())
		return;
	if(GameServer()->Block().Accounts() && GameServer()->Block().Accounts()->ShutdownFlushActive())
	{
	...
	if(GetClanId() > 0)
	{
		GameServer()->Block().Clans()->SaveClan(GetCid(), GetClanId());
	}
...
void CBlockPlayer::ClearCosmetics()
{
...
void CBlockPlayer::DisableCosmeticsForEvent()
{
...
const char *CBlockPlayer::GetPlayerSpecials()
{
	return m_aSpecialsOwned;
}

bool CBlockPlayer::ToggleSpecial(int SpecialIndex)
{
	if(SpecialIndex < 0 || SpecialIndex >= CCosmeticsHandler::NUM_SPECIALS)
		return false;
...
void CBlockPlayer::ProcessWeeklyReward()
{
	if(!IsLoggedIn())
		return;
	...
	// IP-based abuse guard: if another account from the same IP already claimed today, deny
	{
		...
		BlockClientAddr(GameServer()->Server(), GetCid(), aAddrWithPort, sizeof(aAddrWithPort));
		...
		auto It = GameServer()->Block().m_WeeklyRewardClaimedByIp.find(aIpKey);
		if(It != GameServer()->Block().m_WeeklyRewardClaimedByIp.end() && It->second == TodayDate)
		{
			GameServer()->Block().SendChatTarget(GetCid(), "[Daily Reward] You have already claimed today's reward!");
			return;
		}
	}
	...
	// Record this IP as having claimed today
	{
		...
		if(pLastColon)
			*pLastColon = '\0';
		GameServer()->Block().m_WeeklyRewardClaimedByIp[aIpKey] = TodayDate;
	}

	OnPlayerSave(false);
}

void CBlockPlayer::AddPlayerExp(int Amount, bool ApplyMultiplier)
{
	...
	if(LevelsGained > 0)
	{
		CPlayer *pPlayer = GameServer()->Block().GetPlayer(GetCid());
		...
		for(int l = StartLevel + 1; l <= GetPlayerLevel(); l++)
		{
			if(l % 50 == 0)
			{
				...
				GameServer()->Block().SendChatTarget(GetCid(), "[LevelUp+]: You've received 300bp !");
			}
		}
...
void CBlockPlayer::AddExpMultiplier(int ModifierPercent, int Duration)
{
...
void CBlockPlayer::CalculateExpMultiplier()
{
...
void CBlockPlayer::SendBroadcastImp(const char *pMessage)
{
	...
	GameServer()->Block().SendBroadcast(pMessage, GetCid());
}

void CBlockPlayer::Tick()
{
	if(!m_AccountQueryResult.empty() && m_AccountQueryResult.front() && m_AccountQueryResult.front()->m_Completed)
	{
		BlockProcessAccountsResult(*m_AccountQueryResult.front());
		...
		if(Firing)
		{
			...
			if(m_TelekinesisTarget != -1)
			{
				...
				if(pTarget && pTarget->IsAlive())
				{
					...
					pTarget->Block().m_TelekinesisHeld = true;
					pTarget->Block().m_TelekinesisTargetPos = CursorWorldPos;
					pTarget->Block().Core().m_Pos = CursorWorldPos;
					pTarget->Block().Core().m_Vel = vec2(0, 0);
...
void CBlockPlayer::Reset()
{
	...
	GameServer()->Block().BlockTracker().StopTrackPlayer(GetCid());
...
void CBlockPlayer::OnDisconnect()
{
	ClearCosmetics();
}
