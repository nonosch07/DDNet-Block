#ifndef BLOCK_CLANS_H
#define BLOCK_CLANS_H
...
{
	int m_Id;
	char m_ClanName[BLOCK_CLAN_NAME_BUFFER_SIZE];
	int m_Level;
	int m_Experience;
...
struct CClanResult : ISqlResult
{
	...
	char m_ActionOldClanName[BLOCK_CLAN_NAME_BUFFER_SIZE]{};
	char m_ActionNewClanName[BLOCK_CLAN_NAME_BUFFER_SIZE]{};
...
struct CSqlClanRequest : ISqlData
{
	...
	char m_aClanName[BLOCK_CLAN_NAME_BUFFER_SIZE];
	char m_aUsername[12]; // Match SQL schema varchar(11) + null terminator
	int m_AccountId;
	int m_ClientId;
	int m_ClanId;
	int m_NewAuthLevel;
	char m_aNewClanName[BLOCK_CLAN_NAME_BUFFER_SIZE];
...
public:
	...
	/// Creates the clan table if it is missing. Enqueue before LoadAllClans.
	void EnsureSchema();
	static bool EnsureSchemaThread(IDbConnection *pSqlServer, const ISqlData *pGameData, Write w, char *pError, int ErrorSize);
