...
MACRO_CONFIG_INT(SvBlockExperienceMultiplierStacking, sv_block_expirience_multiplier_stacking, 1, 0, 3 /* NUM_EXP_CALC_METHODS - 1, see block/common.h */, CFGFLAG_SERVER, "0-highest, 1-additive, 2-logarithmic, 3-multiplicative")
...
MACRO_CONFIG_INT(SvVotemenuEnabled, sv_votemenu_enabled, 1, 0, 1, CFGFLAG_SERVER, "Enable (1) or disable (0) the block vote menu (shop/cosmetics) globally.")
...
MACRO_CONFIG_INT(SvTDMFreezeTime, sv_tdm_freeze_time, 5, 3, 30, CFGFLAG_SERVER, "Specifies the time required for a player to be frozen in order to die")
...
MACRO_CONFIG_INT(SvZCatchGrenadeAnticamperTime, sv_zcatch_grenade_anticamper_time, 10, 5, 120, CFGFLAG_SERVER, "How many seconds to wait till the player dies/freezes")
...
// Discord integration
MACRO_CONFIG_STR(SvDiscordInvite, sv_discord_invite, 63, "", CFGFLAG_SERVER | CFGFLAG_GAME, "Discord invite shown in the vote menu (empty hides the line)")
...
MACRO_CONFIG_INT(SvWhoisRconTimeoutMs, sv_whois_rcon_timeout_ms, 5000, 100, 30000, CFGFLAG_SERVER, "How long an rcon whois waits for the database before giving up (milliseconds)")
